...
- **Engine:** ruff (line 120, py312, double quotes, LF) — **`ruff format` fait convention et est GARDÉ** (`ruff format --check` dans `make lint`/`make test` et dans le job `Engine Tests`, P4-64 2026-08-09) : lancer `make format` est donc sans danger, il ne churne plus rien · mypy `strict` + `pydantic.mypy` plugin (`ortools.*` ignored) · pytest (`-ra`) + golden fixtures + invariants + hypothesis.
