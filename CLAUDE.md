...
`{lint, phpstan} → blocking-tests → {unit-tests, e2e (Playwright, full stack + Vite)}` · `engine-tests` runs in parallel from the start (no needs) · `build-docker` needs **[blocking-tests, engine-tests] only** — unit-tests and e2e do NOT gate it · `engine-perf` gate runs on main only.
