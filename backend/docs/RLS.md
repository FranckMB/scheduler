...
Last verified @ 2026-08-22 (P4-120 — première vérification stampée de ce fichier, contre le code, **tout juste** : canon par égalité stricte `RlsIsolationTest::testEveryPolicyOnClubIdTablesIsTenantScoped` (`:89`) ✓ · `TENANT_PREDICATE` en constante dans les migrations récentes ✓ · les trois migrations citées existent (`Version20260703120000` activation, `Version20260804120000` SEC-12, `Version20260813130000` policies `admin_all`) ✓ · `app_security.set_club_id` appelée par AUCUN code applicatif ✓ · `TenantConnectionContext::runWithoutTenant` (`:74`) ✓. Le fichier assume sa frontière d'audience — how-to exploitant ici, architecture canonique dans `docs/security/rls.md` — et documente sa propre dérive passée du prédicat : rien à corriger ce jour)
