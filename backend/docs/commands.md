...
| `app:clubs:set-plan` | P1-3 PR A : attribue l'offre `--plan=<code>` (`decouverte`/`essentiel`/`club`/`grand-club`/`sans-limite`/`beta`) à `--club=<id>` — SEULE porte d'attribution (v1 = virement + geste superadmin ; l'offre Bêta n'a pas d'autre chemin par construction). Une entrée du catalogue console PAR offre (`--plan` figé, seul `--club` runtime) |
