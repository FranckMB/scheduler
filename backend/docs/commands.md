...
| `app:demo:create` | P2-4 : crée un club de DÉMONSTRATION depuis `--ffbb=<code>` (`--name` requis) et y REPOINTE le compte animateur (`--animator-email`, défaut demo@clubscheduler.fr ; `--animator-password` requis au premier passage) — adhésions précédentes supprimées (une seule active), populate FFBB synchrone best-effort, club non onboardé (le wizard guidé EST la démo). SEUL chemin qui pose `is_demo`. CLI seulement (arguments libres — hors catalogue console) |
