...
Last verified @ 2026-08-21 (recalé par la livraison d'**AUD-BCK-14** — le repli d'enum silencieux des entrées de calendrier. Re-vérifié DANS CETTE PASSE et rien d'autre : la ligne `CalendarEntry` du tableau des ressources, confrontée à `CalendarEntryStateProcessor` (les trois parsers, l'auto-lien `schoolHolidayId`, la garde `assertValidWeekChild`) et au DTO `CalendarEntryInput` (les trois champs sont bien FACULTATIFS avec `Assert\Choice`, ce qui rend la distinction absent/inconnu nécessaire). ⚠ Vérification volontairement ÉTROITE : ce fichier est un inventaire long, le reste de ses lignes n'a pas été confronté au code cette fois)
