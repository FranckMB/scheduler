...
Last verified @ 2026-08-20 (re-vérifié contre `backend/src/Controller/SocleDeviationController.php`, `backend/src/Service/SocleDeviationCalculator.php`, `backend/src/Service/SocleDeviationResult.php` et `backend/src/OpenApi/CustomRoutesOpenApiFactory.php` — nouvelle route `GET /api/schedules/{id}/socle-deviation` [ADR-0004 PR-5, les écarts nommés] documentée §3 ; cible = la VERSION, appariement chronologique, raison dérivée de `PeriodConstraintSelector` avec `null` assumé, absence de gate management et de `SeasonScopedWriteInterface`, lecture pure sans `persist`/`flush` — tout confronté au code. Les deux routes voisines `transcribe-from-socle` et `/fill` relues à cette occasion : inchangées. ⚑ Le stamp s'était REMIS à s'empiler (« ; précédemment : … ») malgré DOC-33 — aplati ici. Historique des passes : `git log -p --follow backend/docs/backend-inventory.md`.)
