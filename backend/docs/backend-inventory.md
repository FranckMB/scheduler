...
Last verified @ 2026-08-19 (**rotation de fraîcheur** — re-vérifié contre le code et le snapshot, **rien de faux** : les QUATRE routes `deletion-impact` documentées (venues, teams, coaches, venue_training_slots) correspondent exactement aux quatre paths du snapshot OpenAPI ✓ · le contrat annoncé **2.12** correspond à `engine/CONTRACT_VERSION` ✓. ⚠ Une fausse alerte évitée en vérifiant plutôt qu'en comptant : un `grep -c` rendait 3 au lieu de 4 — une seule ligne portait trois routes) — *(historique des passes précédentes retiré le 2026-08-19, audit DOC-33 : 48 entrées empilées faisaient de ce stamp un mur de 23 Ko qu'un agent traverse avant d'atteindre le contenu. Il vit dans git : `git log -p --follow backend/docs/backend-inventory.md`)*
