...
final readonly class AdminJobRunner
{
    ...
    public function run(AdminJobDefinition $definition, string $source, ?string $superAdminId, callable $execute): int
    {
        ...
        try {
            $runId = $this->store->start($definition, $source, $superAdminId);
            try {
                $exitCode = $execute();
