...
/**
 * AUD-DOC-26 — la liste des blocking-tests de `CLAUDE.md` §4 doit être EXACTEMENT celle des
 ...
 * `ScheduleConstraintBuilderOverlayTest` bloquaient sans y figurer. §4 est le premier fichier
 * que lit un agent : quand il ment sur ce qui garde le dépôt, tout ce qui en découle est faux.
 *
...
final class BlockingTestsListMatchesCiTest extends TestCase
{
    ...
    private const string INDEX = self::ROOT . '/CLAUDE.md';

    /** Le job dont les steps FONT le gate. */
    private const string JOB = 'blocking-tests';

    /** Bornes du paragraphe de §4 qui porte la liste (hors avertissements alentour). */
    ...
    public function testEveryGatingStepIsAnnouncedInTheIndex(): void
    {
        ...
        self::assertSame([], array_values($missing), \sprintf(
            "Ces tests BLOQUENT (steps du job « %s ») mais ne figurent pas dans la liste de CLAUDE.md §4 :\n  - %s\n"
    ...
    public function testEveryTestAnnouncedInTheIndexActuallyGates(): void
    {
        ...
        self::assertSame([], array_values($phantom), \sprintf(
            "CLAUDE.md §4 annonce ces tests comme bloquants, mais AUCUN step du job « %s » ne les lance :\n  - %s\n"
    ...
    /**
     * Les classes citées par la liste de CLAUDE.md §4.
     *
     * @return list<string>
     */
    private function testsListedInTheIndex(): array
    {
        ...
        self::assertIsInt($start, \sprintf('Le paragraphe « %s » a disparu de CLAUDE.md §4.', self::LIST_OPENS));

        $end = strpos($index, self::LIST_CLOSES, $start);
        self::assertIsInt($end, 'La fin de la liste (« Detail: ») a disparu de CLAUDE.md §4.');
        ...
        self::assertNotEmpty($classes, 'La liste de CLAUDE.md §4 ne cite plus aucun test.');
