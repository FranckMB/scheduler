...
12. **School holidays = global reference** — `school_holiday_period` has **no club_id / no RLS** (public national data, like `sport`). Seeded from a versioned JSON (`app:school-holidays:seed`, idempotent by `(zone, holidayType, schoolYear)`) — **no network at runtime**. Read via `GET /api/school-holidays` (filtered by the club's `schoolZone` + a window). `Club.schoolZone` is derived best-effort from the FFBB code by `SchoolZoneResolver` at register (`app:clubs:backfill-school-zone` for existing clubs, dry-run by default) — null when undecidable, then manually editable. ⚠ FFBB-code→department format is **not officially verified**; derivation degrades to null + manual entry.
