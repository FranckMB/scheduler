...
describe("tagLabel", () => {
  it("maps system tags to readable French labels, falls back to the raw name", () => {
    ...
    // Volet A : ADULTE (= +18) et SENIOR (= +22) sont deux tags distincts, la borne dans le nom.
    expect(tagLabel("ADULTE")).toBe("Adulte (+ de 18)");
    expect(tagLabel("SENIOR")).toBe("Senior (+ de 22)");
    expect(tagLabel("COMPETITION")).toBe("Compétition (hors loisir)");
...
describe("groupTagsByAxis", () => {
  it("orders Genre, Niveau, Âge; drops empty axes; sorts by label; falls back to Autres", () => {
    ...
    // Âge sorted by display label: "Adulte (+ de 18)" (ADULTE) before "U15".
    expect(groups[2].tags.map((t) => t.name)).toEqual(["ADULTE", "U15"]);
  });
