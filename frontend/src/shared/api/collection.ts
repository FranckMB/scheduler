 ...
 */
export async function collection<T>(path: string, searchParams?: Record<string, string>): Promise<T[]> {
 ...
 */
export async function collectionAll<T extends { id: string }>(path: string, searchParams?: Record<string, string>): Promise<T[]> {
