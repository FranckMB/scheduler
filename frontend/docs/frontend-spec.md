...
page » : armement CORRIGÉ + blocage DIFFÉRÉ à 250 ms** : `app/ActionVeil.tsx` (dérivé `blocking`) +
...
armé depuis `features/wizard/store.ts` et `features/planning/store.ts` — tous confrontés au code. Le
...
gardée par le NR `app/ActionVeil.test.tsx`. La règle et la liste des exemptions vivent dans
