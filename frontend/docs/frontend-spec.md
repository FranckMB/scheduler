...
Last verified @ 2026-08-21 (recalé — **quand l'écran d'attente de génération s'affiche (lot C
PR-1)** : nouveau dérivé `showGenerationWaiting = isGenerating || scopeInFlight` confronté à
`planning/PlanningPage.tsx` (dérivation, branche de rendu, et les HUIT gardes qui le lisent
désormais au lieu d'`isGenerating` seul), plus la fenêtre locale de
`wizard/steps/GenerateStep.tsx` — `showPlanning` relu et vérifié INCHANGÉ (le correctif du
2026-08-19 tient). Le §6.7 bis (écarts NOMMÉS, P2-44 PR-5) relu à cette occasion : inchangé.
Historique des passes : `git log -p --follow frontend/docs/frontend-spec.md` (un stamp
