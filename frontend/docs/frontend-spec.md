...
Last verified @ 2026-08-21 (recalé — **le VOILE bloquant (lot C PR-2), règle « changement de page »
CORRIGÉE** : `app/ActionVeil.tsx` + `ActionVeil.css`, son montage dans `app/providers.tsx` (Toaster
laissé HORS du wrapper inert), les `meta.veil` de `features/planning/queries.ts` et
`features/wizard/queries.ts`, `VerdictAbandonedError` dans `features/planning/api.ts`, et le nouveau
déclencheur de transition `shared/stores/navTransitionStore.ts` armé depuis `features/wizard/store.ts`
et `features/planning/store.ts` — tous confrontés au code. Le contexte « page » ne s'arme plus sur le
simple montage d'un écran, seulement sur une transition d'étape/vue : cf. la puce, et le NR
`app/ActionVeil.test.tsx`. La règle et la liste des exemptions vivent dans `frontend/AGENTS.md` (ce
document POINTE, il ne recopie pas). La puce voisine « quand l'écran d'attente s'affiche » (PR-1)
relue : inchangée. Historique des passes : `git log -p --follow frontend/docs/frontend-spec.md` (un
stamp REMPLACE, il ne s'empile pas — DOC-33).
