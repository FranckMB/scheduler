...
Last verified @ 2026-08-22 (édition lot PASSERELLES PR-3 — le panneau Mutualisation change de SOURCE : la short-list « Équipes liées » se dérive désormais des passerelles SERVIES (`useTeamLinks`), l'heuristique catégorie+genre `rankCandidates`/`gendersCompatible` étant supprimée. Re-vérifié contre le code : `features/wizard/steps/MutualisationPanel.tsx` utilise `splitByLinks`/`teamsLinkedTo` (`features/wizard/lib/sharedTraining.ts`) sur `useTeamLinks` (`@/features/matches/queries`) ✓ ; le bouton « Gérer les passerelles » monte `HabitsLinksButton` (`features/matches/HabitsLinksButton.tsx`) ✓ ; l'état vide « Aucune passerelle déclarée » est rendu quand `teamLinks` est vide ✓. Le reste du fichier non re-vérifié cette passe — un stamp REMPLACE, l'historique des passes précédentes vit dans git)
