...
| **COACH_AVAILABILITY** | `coachId` + `unavailableDays` | **HARD** (épinglé) | Lionel indispo vendredi |
...
| `unavailableDays` | ✅ coach | passe | ✅ union, dur | ✅ **aligné** |
...
| **`availableDays`** (« coach dispo **uniquement** tel jour ») | ❌ non émis | — | ✅ compris (whitelist, intersection) | 🟠 **scission A** : engine sait, le front n'expose pas |
...
- **🟠 Scission A — l'engine sait, le front n'émet pas** : `forcedDays` (au moins une séance tel jour), `availableDays` (coach dispo uniquement tel jour), `preferredDays`. **Feature possible** : exposer ces modes dans le wizard (petit ajout UI + cellule de matrice).
