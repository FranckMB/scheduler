...
- **Une feature = un dossier** `src/features/<x>/` avec `{api,queries,store}.ts` + ses composants. Features livrées :
  ...
  - **`matches`** — **module matchs** (`/matchs`) : grille week-end de placement des rencontres domicile (gardes fenêtre d'accès/indisponibilité, pré-remplissage par habitude, blocs fantômes « fenêtre protégée », cellules cliquables + cadenas d'ancre), bouton « Placer automatiquement » (`POST /api/fixtures/place`, synchrone — raisons de non-placement nommées dans la liste À placer), boucle manuelle au panneau (déplacer/dé-placer/verrou↔solveur/échanger/modifier/supprimer), diagnostic gradué (sévérités serveur groupées, angles morts repliés), bande « À l'extérieur » (salle FBI, heure estimée), vue « week-end type » (habitudes sans dates, lecture seule), import FBI (`ImportFbiDialog`, une passe), appariement « Engagements FFBB » (`FfbbEngagementsDialog`, confirmation en bloc, pré-rempli inter-phases), éditeurs « Accès match » (`MatchWindowsEditor`, partagé wizard) et « Habitudes & passerelles » (`HabitsLinksDialog`, inférence suggérée).
