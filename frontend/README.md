...
- **Une feature = un dossier** `src/features/<x>/` avec `{api,queries,store}.ts` + ses composants. Features livrées :
  ...
  - **`matches`** — **module matchs** (`/matchs`) : grille week-end de placement des rencontres domicile (gardes fenêtre d'accès/indisponibilité, pré-remplissage par habitude, blocs fantômes « fenêtre protégée »), bouton « Placer automatiquement » (`POST /api/fixtures/place`, synchrone — raisons de non-placement nommées dans la liste À placer), radar de conflits (coach + gymnase indisponible + passerelles, heures extérieures estimées par habitude), import FBI (`ImportFbiDialog`, une passe), éditeurs « Accès match » (`MatchWindowsEditor`, partagé wizard) et « Habitudes & passerelles » (`HabitsLinksDialog`, inférence suggérée).
