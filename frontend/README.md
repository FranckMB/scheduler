...
- **Une feature = un dossier** `src/features/<x>/` avec `{api,queries,store}.ts` + ses composants. Features livrées :
  ...
  - **`matches`** — **module matchs** (`/matchs`) : grille week-end de placement des rencontres domicile (sélecteur restreint aux gymnases de match + gardes fenêtre d'accès/indisponibilité), radar de conflits (coach + gymnase indisponible), import FBI (`ImportFbiDialog`, export global club en une passe : analyse → correspondances Division↔équipe → import), éditeur « Accès match » (`MatchWindowsEditor`, partagé avec le wizard).
