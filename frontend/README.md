...
- **Une feature = un dossier** `src/features/<x>/` avec `{api,queries,store}.ts` + ses composants. Features livrées :
  ...
  - **`matches`** — **module matchs** (`/matchs`) : grille week-end de placement des rencontres domicile, radar de conflits coach/joueur, import FBI (`ImportFbiDialog`, export global club en une passe : analyse → correspondances Division↔équipe → import).
