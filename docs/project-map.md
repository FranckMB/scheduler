...
Last verified @ 2026-08-20 (**rotation de fraîcheur, P2-44 PR-5** — added `SocleDeviationController` to §2.3's structuring-controllers table (ADR-0004, "les écarts nommés"), verified against `backend/src/Controller/SocleDeviationController.php` and `backend/src/Service/SocleDeviationCalculator.php`; re-checked `CONTRACT_VERSION` `2.13` ✓ against `engine/CONTRACT_VERSION` and §3.3's own claim — this PR touches no engine line. Historique des passes : `git log -p --follow docs/project-map.md` (un stamp REMPLACE, il ne s'empile pas — DOC-33).
