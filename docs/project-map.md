...
- **Operational jobs (SA3-A/B/C):** `cron-runner` exécute `app:jobs:run-due` chaque minute. `AdminJobCatalog` est une allowlist de dix jobs à arguments et horaires fermés (`Europe/Paris`) : reconcile toutes les 10 min, rappels et purges quotidiens, imports vacances/fériés trimestriels. Le tick rattrape au plus le dernier créneau manqué ; `scheduled_for` et un index unique empêchent le doublon par `(job, créneau)`, tandis que le verrou advisory empêche le chevauchement. `AdminJobRunStore` écrit via la connexion `admin` dans `admin_job_run` (aucun privilège `app_user`, aucun output/message d'exception persisté). `GET /api/admin/jobs` rapproche le catalogue du dernier run et expose aussi le prochain passage fiable au dashboard React. La relance manuelle superadmin reste différée.
