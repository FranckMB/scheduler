...
Last verified @ 2026-08-19 (**rotation de fraîcheur** — re-vérifié contre `backend/src/EventListener/TenantFilterListener.php` (priority 7, after the firewall — confirmed) et `frontend/src/features/` (`ls`: matches §feature list exactly, `feedback`/`release-notes` present) ; **corrigé au passage** : la flatten DOC-33 précédente avait laissé un paragraphe entier de l'ancien historique **échapper du commentaire de stamp** et flotter en texte de corps juste après (dupliquant §"Period-owned grid" ligne 61, `PlanVenueClosures`/`dayOverrides`) — supprimé, rien n'est perdu (déjà couvert §Period-owned grid, et dans git)) — *(historique des passes précédentes retiré le 2026-08-19, audit DOC-33 : 2 entrées empilées faisaient de ce stamp un mur de 1 Ko qu'un agent traverse avant d'atteindre le contenu. Il vit dans git : `git log -p --follow docs/project-map.md`)*
