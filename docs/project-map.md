...
Last verified @ 2026-08-20 (**rotation de fraîcheur, P2-44 PR-3** — added `FillPeriodPlanController` to §2.3's structuring-controllers table (ADR-0004, "le comblement"); re-checked `phpunit/phpunit` at `11.5.55` in `backend/composer.lock` ✓, `CONTRACT_VERSION` `2.13` ✓ against `engine/CONTRACT_VERSION` and §3.3's own claim, `TenantFilterListener` priority 7 ✓ against `getSubscribedEvents()`, `rector.php withPhpVersion(80400)` ✓). Historique des passes : `git log -p --follow docs/project-map.md` (un stamp REMPLACE, il ne s'empile pas — DOC-33).
