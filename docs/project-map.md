...
Last verified @ 2026-08-18 (recalé ce jour : the period-owned-grid orphan-pin paragraph now notes P2-38 — a dated closure applies to a plan TRANSVERSALLY, carried by any calendar entry whose window overlaps, not just the plan's own entry (`PlanVenueClosures`) ; precédemment ce même jour : the period-owned-grid orphan-pin paragraph claimed an orphan pin ALWAYS blocks generation — since P2-37 PR1 backend, a venue ENTIRELY closed by a dated closure over the window is DERIVED unavailable and excluded like a disabled venue, non-blocking ; a PARTIAL closure still blocks ; front surfacing (PR2) remains open in roadmap ; précédemment : 2026-08-17 (re-vérifié contre `App\Service\ProductIdentity` + `frontend/src/shared/lib/product.ts` : le produit s'appelle **Amateo**, édité par **Maratech** — P5-15, titre de ce fichier recalé ; l'identité produit/éditeur est une VARIABLE unique de chaque côté, jamais un littéral — détail et exceptions techniques (graine UUIDv5 du moteur, verrou admin, préfixe des sauvegardes, identifiants d'infra, ancienne clé de brouillon coach) : `specs/courantes/etat-des-lieux.md` §2) ; précédemment : 2026-08-17 (re-vérifié ce jour contre `backend/src/MessageHandler/GenerateScheduleHandler.php` + `backend/src/Service/ScheduleConstraintBuilder.php` : P3-21 PR B — le backend ÉMET désormais `previousAssignments` (contrat 2.11, terme de stabilité posé en PR A) depuis `RegenerateController`/`GenerateScheduleHandler`, le champ n'est plus inerte ; injecté APRÈS le hash de snapshot (préférence de convergence, pas une donnée de structure) ; **LOT P3-21 SOLDÉ** ; précédemment ce même jour : contrat backend⇄engine 2.10 → **2.11** — champ d'entrée optionnel `previousAssignments` + terme de stabilité de génération (P3-21 PR A), absent/vide ⇒ payload byte-identique, goldens + score inchangés ; précédemment : 2026-08-16, contrat backend⇄engine 2.9 → **2.10** — compromis nommés sur `/validate-assignments` (P2-32 PR A : nouveau champ d'entrée `reference`, sortie `compromises[]` sur un candidat ACCEPTÉ, delta évalué par le solveur, jamais un poids) ; précédemment ce même jour : 2.8 → 2.9, retrait des champs morts `temporaryLock`/`temporaryLockFor`/`temporaryMinSessionsOverride`, jamais lus par le solveur ; précédemment : 2026-08-15, contrat 2.7 → 2.8, cause STRUCTURÉE d'une séance manquante, P4-99 PR-1))
...
- **Period-owned grid** (#8, 2026-07-24): **VenueTrainingSlot** carries a `schedulePlanId` (null = the season model). At plan birth the whole season grid is **copied** onto the plan (`SchedulePlanProvisioner`/`VenuePeriodGrid::copySeasonalSlots`) — there is **never** a season ∪ period union. **VenuePeriodOverride** is the sparse per-(plan, venue) mode: `DISABLED` (grid kept, venue removed from the engine payload) / `BLANK` (grid emptied) / no row = inherit (going back to inherit re-empties then **re-copies** from the season model). A pin that no longer lands on any slot of that grid blocks generation with a **422** naming venue, day and team (`OrphanPinGuard`, called by `GenerateScheduleController`) — never filtered silently. **Exceptions (non-blocking, the pin is inert): a DISABLED venue** (P3-20) **or a venue ENTIRELY CLOSED over the window by a dated closure** (DERIVED unavailability from closed dates, never stored — P2-37; TRANSVERSAL since P2-38: `PlanVenueClosures` applies a closure carried by ANY calendar entry once its window overlaps the plan's, not just the plan's own entry) — a PARTIAL closure (a single closed day on an otherwise-open venue) still blocks, since the session would otherwise be silently relocated.
...
| `app/solver/constraints.py` | Level-1 hard constraints + `parse_v2_constraints()` + `diagnose_locked_slot_violations()` (P2-9) |
...
- **Services (dev):** PostgreSQL 16 (`clubscheduler-postgres`), Redis 7 appendonly (`clubscheduler-redis`), Mercure hub (`clubscheduler-mercure` — signed with the **dedicated `MERCURE_JWT_SECRET`, never `JWT_PASSPHRASE`**: the two being the same value *was* SEC-06, and `MercureHardeningTest` now blocks its return), Mailpit (`clubscheduler-mailpit`), `pdf-worker` (Node), `php-fpm` + nginx, `engine`, **`messenger-worker`** (consumes the Redis queue — without it a generation stays `PENDING`), **`cron-runner`** (`app:jobs:run-due` every minute), `frontend` (nginx :8081) and the dev helpers `frontend-dev` / `frontend-tooling`. Every service has a Docker healthcheck. Details on the hub: [`security/mercure.md`](security/mercure.md).
