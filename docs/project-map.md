...
Last verified @ 2026-08-19 (**rotation de fraîcheur, P2-44 PR-1** — added `TranscribePeriodPlanController` to §2.3's structuring-controllers table (ADR-0004); re-checked `phpunit/phpunit` at `11.5.55` in `backend/composer.lock` ✓, `CONTRACT_VERSION` `2.12` ✓ against §3.3's own claim). Historique des passes : `git log -p --follow docs/project-map.md` (un stamp REMPLACE, il ne s'empile pas — DOC-33).
