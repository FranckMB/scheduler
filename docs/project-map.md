...
Last verified @ 2026-08-13 (stamp recalé — commit du contenu du 12 franchi minuit, PR #536 à 00:03 ; contenu inchangé depuis) (CONTRACT_VERSION recalé **2.2 → 2.4** : le chiffre cité était périmé de deux versions, aligné sur le fichier source de vérité — bump P2-2 F2a `/validate-assignments`, guards de contrat listés ; précédemment P4-47 : l'énumération des routes custom re-déclarées était partielle et périmée — remplacée par l'invariant « toutes, gardé par un test contre le routeur » ; commande de régen alignée sur celle du test ; précédemment : ajout de `landing/` au layout — dossier vérifié sur disque, contrat `config.js` relu ; périmètre frontend re-vérifié contre `frontend/src/features/`, toujours 11)
