...
Last verified @ 2026-08-19 (rotation de fraîcheur — re-vérifié contre `backend/src/EventListener/TenantFilterListener.php` (priority 7, after the firewall — confirmed) et `frontend/src/features/` : the feature list was STALE, missing `feedback` (P5-6) and `release-notes` (P5-12), both shipped weeks ago — fixed above, and the volatile "11 features" count dropped in favour of `ls`ing the dir) — *(historique des passes précédentes retiré le 2026-08-19, audit DOC-33 : 2 entrées empilées faisaient de ce stamp un mur de 1 Ko qu'un agent traverse avant d'atteindre le contenu. Il vit dans git : `git log -p --follow docs/project-map.md`)*
