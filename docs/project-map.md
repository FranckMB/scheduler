...
- **Operational jobs (SA3-A):** `cron-runner` passe ses huit commandes horaires par `app:jobs:run <clé>`. `AdminJobCatalog` est une allowlist à arguments fixes ; `AdminJobRunner` prend un verrou advisory PostgreSQL par clé et `AdminJobRunStore` écrit via la connexion `admin` dans la table globale `admin_job_run` (aucun privilège `app_user`, aucun output/message d'exception persisté). L'API/UI, les imports annuels et les relances arrivent dans les lots SA3 suivants.
