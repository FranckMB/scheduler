...
### 3.3 Contract
`engine/CONTRACT_VERSION` holds the version (currently `2.2` — bumped for the `/place-matches` second problem, P1-4 PR D ; `2.1` had added the coach hour-window fields, #195). Backend syncs the version via `GET /` and enabled implicit rules via `POST /implicit-constraints`. No codegen — Pydantic (engine) ⇄ payload (backend) are kept in sync manually; `ContractSchemaTest` (backend) is the guardrail.
