...
Last verified @ 2026-08-22 (recalé au bump de contrat **2.13 → 2.14** (lot PASSERELLES PR-1 : bloc optionnel `teamLinks` accepté-non-consommé) — la version courante du payload backend↔engine portée à 2.14 dans la carte du contrat. Précédemment recalé par **P4-118** — la conf nginx du frontend devient UNIQUE. Re-vérifié DANS CETTE PASSE, et une affirmation FAUSSE corrigée : la ligne « Edge routing » décrivait encore un proxy `/engine/` rendu « en dev seulement » par le conteneur `frontend`, et une conf prod qui « drops that location » — les DEUX étaient périmées. Confronté au code : `frontend/vite.config.ts:40-59` (proxy dev = `/api`, `/exports`, `/.well-known/mercure`, et le commentaire FRT-17 qui acte l'absence de `/engine`) et `docker/frontend/nginx.conf` (locations réelles : `/health`, `/index.html`, les assets, `/.well-known/mercure`, `/api/`, `/bundles/`, `/exports/`, `/` — **aucune** `/engine/`). ⚠ Vérification volontairement ÉTROITE : le reste de la carte n'a pas été reconfronté au code cette fois)
