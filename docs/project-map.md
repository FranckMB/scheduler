...
- **Schedule** — per-club-season run: `status` (`DRAFT|PENDING|GENERATING|COMPLETED|FAILED|VALIDATED`), `score`, snapshot data/hash, inline solver metrics, `calendarEntryId` (non-null ⇒ this schedule is a **period overlay**, not a season plan). `SolverMetric` stores the immutable per-attempt history for SA1. `VALIDATED` lifecycle is a structuring axis (§7.1).
- **Coach**, **Venue**, **Constraint**, **Reservation** (durable team→slot HARD pins, base/overlay via `calendarEntryId`, fed to the engine as `slotTemplates` — distinct from ScheduleSlotTemplate which stores solve *results*), **ScheduleSlotTemplate**, **ScheduleDiagnostic**, **TeamCoach** / **CoachPlayerMembership** (coach↔team / player↔team links), …
...
- **`GenerateScheduleMessage`** (`scheduleId`, `clubId`, `timeoutSeconds`=650) → **`GenerateScheduleHandler`** (orchestration only, ~290 l — BCK-04): acquire `ClubGenerationLock` → frozen snapshot (`ScheduleConstraintBuilder`) → `EngineClient.solve()` (`POST http://engine:8000/generate`) → import via `ScheduleResultImporter` → `SolverMetricsMapper` + `ScheduleDiagnosticsRecorder` → **flush (persist result)** → `ScheduleProgressPublisher.publishSafely()` (Mercure **best-effort** — the frontend polls as fallback, so a publish failure never discards a persisted solve).
- **Terminal-status guarantee (BCK-01):** a schedule never freezes in `PENDING`/`GENERATING`. Three nets: (1) the handler catch-all clears the dirty unit-of-work and marks `FAILED` on any uncaught error; (2) `ScheduleGenerationFailureListener` (`WorkerMessageFailedEvent`, `willRetry()===false`) terminates permanently-failed messages (e.g. lock-exhaustion); (3) `app:schedules:reconcile-stuck` fails `GENERATING` schedules older than `--older-than` minutes (worker crash/OOM) — executed hourly by the `cron-runner` compose service. PENDING is left to nets (1)/(2) to avoid racing a legitimately-queued message.
- **Operational jobs (SA3-A/B):** `cron-runner` passe ses huit commandes horaires par `app:jobs:run <clé>`. `AdminJobCatalog` est une allowlist à arguments fixes et cadence descriptive ; `AdminJobRunner` prend un verrou advisory PostgreSQL par clé et `AdminJobRunStore` écrit via la connexion `admin` dans la table globale `admin_job_run` (aucun privilège `app_user`, aucun output/message d'exception persisté). `AdminJobMonitoringService` rapproche le catalogue du dernier run pour `GET /api/admin/jobs`, affiché en lecture seule dans le dashboard React. Les imports annuels, le prochain run fiable et les relances arrivent dans les lots SA3 suivants.
