...
| **Source de placement** (`Fixture.placementSource`) | Qui a posé heure+salle d'un match domicile : `MANUAL` (tout geste API du gestionnaire — avec `SUBMITTED`/`VALIDATED`, c'est une **ancre FIXED** que le solveur ne bouge jamais) ou `SOLVER` (re-plaçable au prochain solve, bonus stabilité). `null` legacy = manuel. ADR-0003 |
...
| **Tag système** (`TeamTag`) | Étiquette auto-assignée (U9…U21, SENIOR, JEUNE, EMB, BABY, FEMININE…) groupée par **axe** GENRE/NIVEAU/ÂGE. Cible de groupe des contraintes (`targetTag`). Les **tranches d'âge sont exclusives** — une équipe n'en porte qu'une, dérivée de sa catégorie par `TeamTagService::determineTagNames`. ⚠ Les bornes portent sur `ageMax`, **pas** sur les noms du catalogue : **BABY** = `ageMax <= 7`, ou, à `ageMax` absent, tout nom contenant « baby » (donc « Baby basket », mais aussi une catégorie importée sans âge) · **EMB** = `ageMax <= 12` (U9-U11 au catalogue, mais une catégorie custom d'`ageMax` 12 y tombe aussi) · **JEUNE** = `ageMin <= 18` (U13-U18) · **SENIOR** = `ageMin >= 19` (U21 et au-delà). Dire « BABY = U5-U7 » est donc un raccourci de catalogue, pas la règle. ⚠ Une catégorie sans âge et sans « baby » au nom (« Loisir ») ne tombe dans **aucune** tranche : elle est hors de portée d'une contrainte par âge. |
...
| **Mode de gymnase** (`VenuePeriodOverride`) | Réglage **sparse** par (plan, gymnase) agissant sur cette copie : **DÉSACTIVÉ** = la grille est conservée mais le gymnase sort du payload envoyé à l'engine · **VIERGE** = la grille est vidée · **pas de ligne = hériter** (revenir à hériter revide puis **RECOPIE** depuis le modèle de saison). |
| **Épinglage orphelin** | Un verrou HARD ou une réservation qui ne retombe plus sur aucun créneau de la grille de la période. **Bloque la génération en 422** en nommant le gymnase et le jour (`OrphanPinGuard`) — jamais filtré en silence. |
