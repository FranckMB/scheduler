...
| **Épinglage orphelin** | Un verrou HARD ou une réservation qui ne retombe plus sur aucun créneau de la grille de la période. **Bloque la génération en 422** en nommant le gymnase, le jour et l'équipe (`OrphanPinGuard`) — jamais filtré en silence. **Exceptions (non bloquantes, l'épinglage est inerte) : un gymnase DÉSACTIVÉ** (`VenuePeriodOverride`, P3-20) **ou ENTIÈREMENT FERMÉ sur la fenêtre par une fermeture datée** (indisponibilité DÉRIVÉE des dates fermées, jamais stockée, P2-37) — une fermeture PARTIELLE (un seul jour fermé d'un gymnase par ailleurs ouvert) reste bloquante. |
...
## Payload backend↔engine (contrat `CONTRACT_VERSION`, actuel 2.12)
