...
| **Niveau** (`Team.level`) | Le niveau FÉDÉRAL sous lequel l'équipe est inscrite : REGIONAL, DEPARTEMENTAL, ELITE… Figé dès qu'elle est **engagée**. Rien à voir avec le rang/tier. |
| **Équipe engagée** | Elle porte ≥1 match de statut ≠ `UNPLACED` (un match à l'**extérieur** compte : il naît `PLACED`, son horaire étant imposé par l'adversaire). Ses matchs sont connus de la fédération : elle ne peut plus être **supprimée** ni changer de **niveau**. Nom, rang, créneaux et gymnase restent libres — « une équipe qui joue peut être déplacée, pas supprimée ». Exposé par `TeamResource.isEngaged` ; la règle vit dans `TeamEngagementGuard`. |
