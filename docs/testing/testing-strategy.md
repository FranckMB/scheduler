...
Last verified @ 2026-08-19 (**rotation de fraîcheur, P2-44 PR-1** — graphe des jobs reconfronté à `ci.yml` : les SEPT jobs sans `needs` ET dont rien ne dépend — `frontend`, `secrets-scan`, `semgrep`, `dependency-audit`, `rector`, `engine-semantics`, `smoke-tests` — sont bien isolés (`lint`/`phpstan` alimentent `blocking-tests`, `engine-tests` alimente `build-docker`, tous deux donc NON isolés malgré l'absence de `needs`) ✓ ; `e2e: needs blocking-tests` ✓ ; `build-docker: needs [blocking-tests, engine-tests]` ✓. ⚠ **Trouvé en chemin** : `specs/evolution/roadmap.md` réutilisait `P4-120` pour DEUX lignes distinctes (id censé être stable et jamais réutilisé) — la seconde (§5 ci-dessous, les 3 parcours e2e) renumérotée **P4-122** dans les deux fichiers, la première (docs de zone jamais stampés, tracée depuis `etat-des-lieux.md`) garde `P4-120`). Historique des passes : `git log -p --follow docs/testing/testing-strategy.md` (un stamp REMPLACE, il ne s'empile pas — DOC-33).
