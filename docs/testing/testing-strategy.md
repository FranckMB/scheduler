...
Last verified @ 2026-08-22 (recalé — le §3bis, ajouté la veille par la PR-1 de P4-122, portait un stamp au 21/08 alors que son merge a atterri le 22 : le garde compare à la date git de DERNIÈRE ÉDITION, il rougissait donc sur `main`. ⚑ **Piège systémique à connaître** : toute PR rédigée un jour et mergée le lendemain fera rougir `DocStampFreshnessTest` — recaler le stamp au moment du merge, pas de la rédaction. Affirmations du §3bis re-vérifiées à cette occasion : la fixture enregistre bien les réponses `/api/*` ≥ 400 et ne les attache QUE sur échec (`frontend/tests/e2e/fixtures.ts:32-38`, `testInfo.status !== testInfo.expectedStatus`), et la convention d'import tient — 9 specs prennent `test`/`expect` de `./fixtures` ; les trois occurrences restantes de `@playwright/test` (`matches`, `season-transition`, `width-calibration`) n'importent que le TYPE `Page`, ce que la convention n'interdit pas. Historique des passes : `git log -p --follow docs/testing/testing-strategy.md` — un stamp REMPLACE, il ne s'empile pas (DOC-33).)
...
- **P4-122 — aucun e2e ne construit les 3 plannings « réalité d'un club »** (socle + reprise + adaptation, chacun ancré sur SES propres versions) : `frontend/tests/e2e/journey.spec.ts` couvre le parcours socle seul (register→onboarding→wizard→génération→validation→cockpit). C'est ce test qui aurait rougi sur le repli silencieux saison de l'ancrage génération période (2026-08-19) et sur l'incident Matéo (P5-13) — deux bugs où un écran d'adaptation/génération retombait sur le mauvais plan. Détail : `specs/evolution/roadmap.md` P4-122.
