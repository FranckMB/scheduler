...
- **P4-120 — aucun e2e ne construit les 3 plannings « réalité d'un club »** (socle + reprise + adaptation, chacun ancré sur SES propres versions) : `frontend/tests/e2e/journey.spec.ts` couvre le parcours socle seul (register→onboarding→wizard→génération→validation→cockpit). C'est ce test qui aurait rougi sur le repli silencieux saison de l'ancrage génération période (2026-08-19) et sur l'incident Matéo (P5-13) — deux bugs où un écran d'adaptation/génération retombait sur le mauvais plan. Détail : `specs/evolution/roadmap.md` P4-120.
