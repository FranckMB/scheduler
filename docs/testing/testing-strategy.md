...
Last verified @ 2026-08-19 (**fix ancrage génération période**, §5 gagne le manque **P4-118** — re-vérifié contre `frontend/tests/e2e/journey.spec.ts` : ne couvre que le parcours socle) ; précédemment : 2026-08-19 (**rotation de fraîcheur** — le stamp précédent avouait « aucune autre vérification de fond faite » ; c'est fait. Graphe des jobs confronté à `ci.yml` : la chaîne `lint`+`phpstan` → `blocking-tests` → `{unit-tests, e2e}` et `{blocking-tests, engine-tests}` → `build-docker` est JUSTE ✓. **Le COMPTE était faux** : « trois jobs isolés sans `needs` » alors qu'il y en a SEPT — manquaient `secrets-scan`, `semgrep`, `engine-semantics` et `smoke-tests`, qui sont pourtant des required checks de `main` : un lecteur en concluait qu'ils n'existaient pas ou ne bloquaient rien. `engine-perf` était donné « main only » sans dire qu'il dépend d'`engine-tests` — corrigé)
...
- **P4-118 — aucun e2e ne construit les 3 plannings « réalité d'un club »** (socle + reprise + adaptation, chacun ancré sur SES propres versions) : `frontend/tests/e2e/journey.spec.ts` couvre le parcours socle seul (register→onboarding→wizard→génération→validation→cockpit). C'est ce test qui aurait rougi sur le repli silencieux saison de l'ancrage génération période (2026-08-19) et sur l'incident Matéo (P5-13) — deux bugs où un écran d'adaptation/génération retombait sur le mauvais plan. Détail : `specs/evolution/roadmap.md` P4-118.
