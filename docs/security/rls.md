...
`admin_all FOR ALL TO clubscheduler USING (true) WITH CHECK (true)`. En local `clubscheduler` est
...
`NOSUPERUSER` membre de `clubscheduler` — voit et écrit cross-club pendant qu'`app_user` reste
