...
> ⚠️ **`RlsIsolationTest` ne garde PAS ces exceptions.** Il vérifie `rls_enabled`, `rls_forced` et `policies > 0` — **jamais la PORTÉE** des policies. Un `USING (true)` posé par erreur sur une table tenant (le chemin probable : copier-coller de la migration `coach_wish_token`, qui est le modèle qu'on imite) **passerait le test sans rougir**. Conséquence pratique : sur ces trois tables, l'isolation ne repose plus que sur **une** couche (le filtre Doctrine) au lieu de deux — une requête native ou un `createQuery` filtre désactivé y fuiterait en silence. Durcissement et test de portée tracés en dette (roadmap §Dette, SEC-12).
