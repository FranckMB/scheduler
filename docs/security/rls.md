...
- **Connexion runtime = `app_user`** (`DATABASE_URL`) : NOSUPERUSER, DML only. Toutes les tables à `club_id` (14) portent `ENABLE` + `FORCE ROW LEVEL SECURITY` et une policy `tenant_isolation FOR ALL TO app_user` :
...
- Tables sans `club_id` (`club`, `app_user`, `plan`, `sport`, `priority_tier`, `reset_password_request`, `team_tag_assignment`, `messenger_*`, `doctrine_migration_versions`) : hors RLS. `Club`/`User` sont protégés au niveau API (SEC-01/02) ; `team_tag_assignment` (jointure) = résiduel assumé, ses deux côtés sont RLS.
