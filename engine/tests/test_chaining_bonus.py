...
A coach who has two back-to-back sessions in the same venue receives
a bonus weighted by the highest-tier team they coach across the two
sessions: S=200, A=20, B=2, C=0.2, D=0.02 (tier_weight / 50).

The bonus is SOFT — it influences the objective but is never a hard
constraint. The solver should prefer chaining higher-tier coaches over
lower-tier ones when both are possible.
"""
...
class TestChainingBonusWeights:
    ...
    def test_chaining_tier_weights_are_tier_weight_divided_by_50(self) -> None:
        """S=200, A=20, B=2, C=0.2, D=0.02 (each is tier_weight / 50)."""
...
class TestChainingBonusIntegration:
    ...
    def test_chaining_bonus_returns_count(self) -> None:
        """add_chaining_bonus should return the number of chaining terms added."""
        model = cp_model.CpModel()
        ...
        count = add_chaining_bonus(
            model, [slot1, slot2],
            teams=[{"id": "team-1", "priority_tier": "A"}, {"id": "team-2", "priority_tier": "B"}],
        )
        assert count > 0, f"Expected at least 1 chaining term, got {count}"
    ...
    def test_empty_assignments_returns_zero(self) -> None:
        """add_chaining_bonus with no assignments should return 0."""
        model = cp_model.CpModel()
        count = add_chaining_bonus(model, [], teams=[])
        assert count == 0, f"Expected 0 chaining terms for empty assignments, got {count}"

    def test_single_assignment_returns_zero(self) -> None:
        """add_chaining_bonus with a single assignment should return 0
        (no consecutive pair possible)."""
        model = cp_model.CpModel()
        slot = _assignment(
            model, "slot1",
            team_id="team-1", slot_id="1:18:00", venue_id="venue-1",
        ...
        count = add_chaining_bonus(model, [slot], teams=[{"id": "team-1", "priority_tier": "A"}])
        assert count == 0, f"Expected 0 chaining terms for single assignment, got {count}"
