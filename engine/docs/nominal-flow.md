...
## 1. Le backend construit le payload (contrat 2.4)

Quand un utilisateur clique sur "Generer l'emploi du temps" dans le frontend, le backend assemble un objet JSON conforme au schema `ScheduleInputSchema` (version de contrat **2.4**, fichier `engine/CONTRACT_VERSION`). Voici la structure complete, avec des explications inline.
...
- **`version`** : version du contrat (actuellement `2.4`). Le moteur ne compare que le **MAJOR** : `"2.0"` et `"2.1"` passent tous les deux ; un payload `1.x` ou `3.x` est refuse.
...
1. **Backend** : construit le payload (contrat 2.4) a partir des entites du club (equipes, salles, entraineurs, contraintes)
