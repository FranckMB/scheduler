...
| **P2 — Revue guidée** 🟡 **en cours** | **(1) Re-datation des événements club ✅ livré (P2-PR1, 2026-07-07)** : après « Préparer la saison suivante » (succès ou 409-successeur), le dialog « Reconduire les événements » liste les events (kind=event, non-ignorés) de N — garder (coché) + nouvelle date suggérée **+364 j** (même jour de semaine, durée préservée) — et crée les gardés dans le brouillon via l'API calendar_entries standard (`X-Season-Id` explicite par requête, prime sur le store ; **zéro endpoint/migration**). Sautable (« Plus tard ») ; ré-ouverture = relancer « Préparer » tant que N+1 n'a aucun event (auto-skip sinon). Au passage : **fix du chemin 409→bascule** (ky 2.x consomme le body d'erreur → `error.data`/`serverBody` stashés côté client ; `existingSeasonId` était illisible → la bascule sur le successeur existant était silencieusement cassée). **(2) Wizard de revue par type — ABANDONNÉ (décision utilisateur 2026-07-07)** : le gestionnaire est le premier informé des départs/changements de son club ; l'écran récap de l'assistant existant suffit — la revue forcée n'ajoute rien (à rouvrir seulement si un club pilote oublie des choses à la transition). · **(3) Alertes d'anticipation ✅ livré (P2-PR2, 2026-07-07)** : fenêtre **[15 mai, 15 juillet[** tant qu'aucune saison N+1 n'existe — (a) **e-mail** aux managers via le cron `app:seasons:remind-transition` (3 relances : buckets J-61/J-30/J-14 avant le pivot, rattrapage si run manqué, dédup par `transition_reminder_log(season_id, threshold)` — table sans club_id, patron period_reminder_log) ; (b) **bannière permanente** `SeasonTransitionBanner` (dérivée 100% de `/api/me`, non-dismissible, CTA ouvrant le ConfirmDialog du SeasonSelector via `transitionUiStore` partagé). Dès que N+1 existe : plus d'alerte, mail comme bannière. Éventuel report des tags custom (dépend de la dette `TeamTagService`). | Le **confort** qui rend la transition guidée et sans oubli. Sur P1. |
