...
| **Console super-admin** (monitoring santé/usage/conversion + métriques par club & globales + data ops auto + actions d'exploitation) | 🟡 | idée produit (2026-07-09) | **SA0 à SA3 livrés (2026-07-16)** → vérité courante [`../courantes/superadmin-auth.md`](../courantes/superadmin-auth.md), suite dans [`console-superadmin.md`](console-superadmin.md). Reste **SA4 → SA5**. **🔴** |
...
| P2-7a | **Périmètre engagé — une équipe qui joue est intouchable** | ✅ | S | **Livré (2026-07-16)** : `TeamEngagementGuard` — une équipe portant ≥1 match ≠ `UNPLACED` (extérieur compris : il naît `PLACED`) ne peut plus être **supprimée** (sinon `purgeChildrenOfTeam` emporte ses matchs, connus de la fédé) ni changer de **`level`** (elle y est inscrite). Rang/tier, `isActive`, nom et créneaux restent libres. Une seule définition, consommée par la garde d'écriture ET par `TeamResource.isEngaged` (batché) dont le front grise les deux gestes. → [`module-matchs.md`](../courantes/module-matchs.md) |
