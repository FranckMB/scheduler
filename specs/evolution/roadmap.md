...
| P2-5 | **Découpage hebdomadaire des overlays + écarts types-de-planning (E1, E2, E4)** | 🟠 | L | Modèle validé 2026-07-12 ([`types-de-planning.md`](../courantes/types-de-planning.md)) : la **semaine** est l'unité hors socle. E1 : overlay = semaines auto englobant l'indispo + notification des suivants ; reprise = **choix des semaines** dans les vacances (N cochées ensemble = identiques). E2 : lever l'exclusion été (`isAdaptableHoliday`). E4 : séances/équipe ajustables dans le flux overlay (moteur OK, UI manquante). **E3 (défaut Fanion + importantes) ✅ livré 2026-07-12.** À cadrer lot par lot, validation besoin avant chaque PR |
| P2-6 | **Pattern « Plan » premier-ordre + nommage (ex-E6)** | 🟠 | L | Décision 2026-07-12 (V0, table rase sauf socle) : introduire une **entité `Plan`** (type socle/overlay, ancre season/calendarEntry, **nom public**, `chosenVersionId`) ; les `Schedule` deviennent des **versions** (auto « Vn - date »). En-tête/liste/export lisent le **nom du Plan** ; le sélecteur montre les versions. Corrige le nommage (ex-E6, abandonné car placé sur la version) — voir [`types-de-planning.md`](../courantes/types-de-planning.md). Design à cadrer avant code |
