...
| P5-3b | **Turnstile sur register — le volet restant de l'anti-abus public** | 🟡 | S | Le volet anti-énumération (P5-3a) est livré le 2026-08-13 — reste le captcha. Décisions fondateur DÉJÀ prises (2026-08-13) : **livraison inerte par défaut** (clé vide = désactivé, patron `FFBB_MEILISEARCH_TOKEN` `services.yaml:16`) ; le fondateur crée le compte Cloudflare et pose les clés quand il veut — **trace à laisser dans la checklist de mise en prod** (doc Scaleway local) pour ne pas oublier les clés dans `.env.prod`. Pièges instruits par la recon : la CSP frontend interdit délibérément tout hôte tiers (`docker/frontend/csp.conf:5-6`) → exception `challenges.cloudflare.com` en `script-src` ET `frame-src`, gardée par un test type `sentryCspGuard` (`vite.config.ts:19-24`) ; aucun `ARG`/`ENV` `VITE_*` dans `docker/frontend/Dockerfile` → chaîne à créer pour la sitekey (ou servie au runtime par le backend) ; e2e `auth.spec.ts` + `registerAndVerify` à préserver (off en dev/test). `EnvHygieneTest:33` à étendre si `TURNSTILE_SECRET` entre dans `.env.prod`. Axe auth §7.1 → NR |
