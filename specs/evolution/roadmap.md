...
| P2-7 | **Rouvrir le plan SEASON emporte les matchs** | 🟠 | M | **Décision fondateur (2026-07-16)** : « le plan SEASON est la base de tout ; s'il change, toute la saison est impactée ». Rouvrir détruit déjà les plans secondaires (inv. 14, confirmation 409 `overlays_exist`) ; il doit détruire **aussi les matchs** de la saison, sous la même confirmation. **Lot dédié assumé** (décision 2026-07-16) : c'est un comportement NOUVEAU — rouvrir n'a jamais touché aux matchs — qui demande sa propre confirmation (annoncer le nombre de matchs perdus, pas seulement les plannings) et ses propres tests ; l'ajouter en fin de boucle de revue de la bascule était le moment de se tromper. **Interim livré par la bascule** : le module matchs est gaté sur le pointeur, donc un reopen le rend inaccessible sans rien détruire — état transitoire cohérent avec la cible, à remplacer par la destruction confirmée. |
