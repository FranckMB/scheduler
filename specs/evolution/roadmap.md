...
| P4-90 | **Cache de build `type=gha` dans `ci.yml` — reste du lot phpredis** | ⚪ | S | La cause racine du flake (téléchargement GitHub de phpredis) est levée depuis que l'image passe par PECL — mais chaque run CI rebuilde toujours l'image php ~10 fois (8 jobs, runners éphémères, aucun `cache-from/cache-to`). Le pattern existe déjà dans `deploy.yml` (`docker/build-push-action` + `type=gha` par scope) ; le transposer à `ci.yml` demande de toucher les 8 jobs qui font `docker compose up`/`build` (setup-buildx + réutilisation d'image), d'où le report — gain = minutes de CI, plus aucun enjeu de fiabilité. Voisin toujours ouvert : le `docker pull` gitleaks de `secrets-scan` (Docker Hub, 503 le 2026-08-11) |
...
| P4-91 | **Le lien de reset se construit depuis le header Host — password reset poisoning possible** | 🟠 | XS | Remonté par la security-review de P5-3a (2026-08-13), PRÉ-existant : `PasswordController.php:54` bâtit le lien avec `$request->getSchemeAndHttpHost()` (dérivé du Host entrant) alors qu'`AuthController` utilise `FRONTEND_BASE_URL` partout — et **aucun `trusted_hosts`** n'est configuré (`framework.yaml`). Scénario : un `POST /api/password/forgot` forgé avec `Host: attaquant.tld` sur l'email d'une victime → l'email LÉGITIME reçu porte un lien vers le domaine de l'attaquant → le clic livre le token (TTL 1 h). Fix : construire depuis `FRONTEND_BASE_URL` (aligne sur AuthController, 1 ligne) ; `trusted_hosts` en défense en profondeur. Axe auth §7.1 → compléter `PasswordResetEnumerationTest` (le lien ne suit PAS le Host entrant) |
...
| P5-6 | **Canal signalement — cadré ET tranché, prêt à implémenter** | 🟡 | S/M | Besoin fondateur 2026-08-09, cadrage + **6 décisions D1-D6 TRANCHÉES le 2026-08-13** : [`canal-signalement.md`](canal-signalement.md) — in-app à DEUX portes (contextuelle + burger libre), contexte maximal copié (redondance assumée : le signalement survit à la suppression du planning), signé/tout membre, digest quotidien, liste+statut en console SA **+ panneau qualité de service** (délai dépôt→traité moyenne/p95, volume par topic, âge du plus vieux non traité — §3ter, zéro collecte en plus). Anti-scope : pas de tickets/chat/upload. Prochaine étape : plan (full lane — nouvelle entité tenant → NR d'isolation standard) |
| P5-12 | **Journal de mise à jour + modale « quoi de neuf »** | 🟡 | S/M | Détaché de P5-6 (ajout fondateur 2026-08-13, §3bis du cadrage) : entrées datées **curées au rythme du fondateur** (pas par merge — le coût réel est ÉDITORIAL : écrire des notes en français utilisateur), page consultable dans le burger, modale une-fois-par-nouveauté-non-vue (marqueur « vu » par user), crédit anonyme des signalements corrigés. L'email « traité » de P5-6 pointera dessus. ⚠ Garde-fou hérité : pas de lien automatique bug→release (territoire tickets, exclu) |
| P5-11 | **Corrélation technique : request-id + logs structurés** | ⚪ | S | Détaché de P5-6 (D5, fondateur : « lot séparé mais je le veux ») : rien ne relie aujourd'hui une erreur front, une entrée Sentry backend et un solve engine — zéro request-id (2 hits `getId()` sans rapport), Monolog ABSENT (logs texte Docker, rétention 30 Mo). Contenu pressenti : request-id généré au front, propagé backend→engine, Monolog JSON avec le contexte (request-id, club, user), corrélation exposée à Sentry (P5-1). Sert le debug général et le support, pas seulement le signalement |
| P5-10 | **Métriques de capacité collectées PAR L'APP — décider les paliers 30/100 clubs sur données réelles** | 🟡 | XS/S | Constat mesure P5-4 (2026-08-13) : `solver_metrics` couvre déjà coûts/tailles/fréquence par club réel — mais il manque (a) `queuedAt`/`solveStartedAt` sur Schedule (l'attente en file se dérive aujourd'hui grossièrement de created/updated) ; (b) **`peakRssMb` auto-rapporté par l'engine** dans `solver_metrics` (petit bump de contrat) — chaque solve de prod devient un point de mesure RAM, répond à P5-4b sans infra de monitoring ; (c) **recalibrer les budgets 60/180/600 s sur le réel** : posés ARBITRAIREMENT sans cas vérifié (fondateur, 2026-08-13) — or BCCL, TOP-30 français, se résout en ~2 s à 8 workers : le p95 réel par taille de club sortira de quelques semaines de `wallTimeMs` en prod, et le critère de dimensionnement est « ça passe avec BCCL ». Axe contrat backend⇄engine touché par (b) → NR contrat standard |
