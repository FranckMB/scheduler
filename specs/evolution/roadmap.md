...
| **Jours fériés au calendrier** (afficher les fériés dans le cockpit) | ⬜ | [API etalab jours-feries](https://calendrier.api.gouv.fr) | **Feature dédiée, découplée de l'import vacances (décision 2026-07-06).** Les fériés **ne sont pas** dans le dataset scolaire (pas de 1er mai / 14 juillet isolés ; ses seules dates 1 jour = ponts/marqueurs, écartés par l'import). **Source cible** = API etalab `jours-feries-france` (ou calcul base-Pâques) : 11 fériés légaux + variantes DOM (Abolition de l'esclavage par territoire…). **Modèle distinct** : un férié = 1 date discrète (national/territorial) ≠ période de vacances zonée → nouvelle entité + rendu cockpit « pastille » (≠ semaine grisée). **Le pont reste une décision gestionnaire** : voyant le jeudi férié, il pose une **fermeture** (`periodType=closure`) du vendredi = semaine à 2 jours off (aucun code pont dédié) · **🟡** |
