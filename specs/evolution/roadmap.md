...
| P4-88 | **Le front redérive des règles métier — l'inventaire est fait, le garde reste à construire** | 🟠 | M | Ouvert le 2026-08-12 en réparant `applicableConstraints` (le panneau annonçait des contraintes que le solveur n'appliquait pas à cette équipe). **Ce n'était pas une instance isolée** : l'inventaire commandé par le fondateur a trouvé **5 redérivations** de règles métier côté front — (1) `planning/lib/applicableConstraints.ts` *(corrigé, passé en miroir DÉCLARÉ)* · (2) **`wizard/steps/PeriodStructure.tsx`** — masquage d'une `CLUB+tag` sans équipe active, état par défaut des cases : ⚠ **duplique le MÊME algorithme tag→équipes que (1)**, commenté « mirrored server-side » **sans aucun test de parité** — **le plus urgent** · (3) `wizard/lib/coachDoubleBooking.ts` — « ⚠ PARITÉ OBLIGATOIRE » déclarée, suites jumelles, mais parité **non mécanique** · (4) `wizard/lib/orphanReservations.ts` (jumeau d'`OrphanPinGuard`) · (5) `matches/lib/matchAccess.ts` (capacité). À confirmer : `wizard/lib/slotOverlap.ts`. **Contre-exemples sains à imiter** : `matches/lib/diagnostic.ts` (« severity emitted by SERVER… never re-derives gravity »), `staleness.ts`, `constraintOrder.ts`. **Le garde N'A PAS été construit, délibérément** (tripwire du brief : >5 cas ⇒ stop) — un garde bricolé sans l'inventaire réel serait passé à vide, le défaut même qu'on combat. **À faire** : (a) unifier l'algorithme tag→équipes entre (1) et (2) — deux copies front du même calcul, c'est la dérive garantie ; (b) trancher le régime de (3)(4)(5) : *le backend dit* ou *miroir déclaré + parité mécanique* (patron `PayloadCapacityMirror` + `CapacityMirrorParityTest`) ; (c) **puis** le garde bidirectionnel — toute redérivation déclarée a son test de parité, **et** aucune redérivation non déclarée n'existe (détection mécanique : fichier branchant sur les valeurs d'un enum métier partagé sans figurer au registre ; réutiliser la liste d'enums de `TsUnionsMatchPhpEnumsTest`). ⚠ **La moitié (c)-(ii) est celle qui compte** : un garde qui ne vérifie que le registre valide ce qu'on a bien voulu déclarer. Falsification obligatoire : réintroduire une redérivation non déclarée doit le faire rougir **en nommant le fichier**. La règle d'intention est déjà posée dans `.claude/rules/frontend.md` |
| P4-87 | **Les autres sources de péremption d'un planning** | 🟠 | M | Livré le 2026-08-12 : une **contrainte** modifiée après génération marque le planning périmé (`constraintsChangedSinceGeneration`). Mais une contrainte n'est pas la seule entrée du solveur — identifiées en livrant, **non traitées** : **gymnases** (ajout/retrait/renommage) · **créneaux et grilles** (la grille possédée par la période, ADR-0002) · **coachs** (disponibilités, affectation aux équipes) · **structure de période** (entrées de calendrier, fermetures, vacances) · **réservations** (pins durables) · **paliers de priorité et tags d'équipe** (une re-dérivation de tag déplace la portée d'une contrainte — cf. `TeamTagSyncListener`). ⚑ Les **équipes** sont déjà **partiellement** couvertes par la bannière `structureDiverged` (compare `generatedTeamCount` au nombre d'équipes du jour) — à unifier avec les deux marqueurs plutôt qu'à empiler un troisième signal. **Le patron est établi** : un listener d'entité par source, le même marqueur, la même bannière qui nomme sa cause, remise à zéro à l'import. **Ne pas les faire toutes d'un coup** : chacune a un périmètre (club+saison ? plan ? période ?) à trancher, et un marquage trop large use la bannière jusqu'à ce que le gestionnaire l'ignore. ⚠ Tant que c'est ouvert, un planning peut être périmé **sans le dire** pour toutes ces raisons — c'est exactement le défaut de confiance qu'on vient de réparer pour les contraintes |
