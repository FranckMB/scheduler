...
| P3-19 | **Le récap réplique l'algèbre de créneaux du moteur — la sortir de PHP** | 🟠 | M | **Constaté 2026-07-31 (revue #341, deux rounds).** P2-9 PR A annonce la capacité au récap en lisant le payload ; pour approcher ce que le solveur peut RÉELLEMENT placer, il réplique deux règles du moteur en PHP (déduplication par `(gymnase, jour, heure)`, rabot `FACILITY_CAPACITY` actif). ⚑ **L'historique justifie la ligne** : sept divergences ont été trouvées en deux rounds, et deux « corrections » ont fait tomber la propriété qui justifie l'exercice — elles SOUS-estimaient l'offre, donc produisaient de fausses alertes sur la branche censée ne jamais mentir. Le miroir a donc été volontairement RÉDUIT aux deux réductions dont on prouve qu'elles gardent l'offre au-dessus du réel ; les verrous HARD ne sont plus répliqués du tout (le moteur y émet un créneau par équipe épinglée, décline la durée, place hors grille — raté dans les deux sens). Conséquence assumée : on sous-avertit. **La vraie sortie** est un calcul CÔTÉ MOTEUR — une capacité pré-solve exposée par l'engine, consommée telle quelle. Aucun garde cross-stack ne protège aujourd'hui les deux règles restantes : un changement d'algèbre côté engine rendrait le récap faux en silence. Preuve : `ValidateConstraintsController::payloadOffer` ↔ `engine/app/solver/model.py:54-56`, `engine/app/main.py:288-293`. |
...
| P4-3 | **Trois composants wizard > 400 lignes** (TeamsStep · ConstraintsStep · VenuesStep) | ⚪ | M | Extraire éditeurs/modales en composants frères, la step reste liste + orchestration. P4-36 et P4-37 (soldés) ont déjà retouché ces fichiers — un futur refactor repart de leur état actuel, plus de risque de collision |
...
| SEC-15 | **Images Docker : durcissement USER non-root** | ⚪ | S | Sorti du triage Semgrep (SEC-14) : `engine`, `pdf-worker` et `php-fpm` tournent en root dans le conteneur. Nuance : php-fpm abandonne déjà root pour ses workers (www-data), engine/pdf-worker sont internes jamais exposés — d'où le report. À faire proprement : `USER` dédié par image + test de la stack COMPLÈTE (droits volumes dev, sandbox Chromium du pdf-worker) — pas un one-liner. Les `nosemgrep` posés sur les 3 `CMD` sautent à la livraison |
...
| P4-71 | **e2e matchs : `ensureValidated` suppose « données complètes → Récap »** | ⚪ | S | **Constaté 2026-08-04 (validation P4-33 — échec IDENTIQUE hôte et docker : pas le runner).** Sur la DB dev non réinitialisée, BCCL atterrit à l'étape 1 du wizard (une étape redevenue invalide au fil des tests du jour) : le spec ne trouve ni « Continuer vers la génération » ni « Lancer la génération » et attend « Valider » 180 s pour rien (`frontend/tests/e2e/matches.spec.ts:26-50`). En CI (DB fraîche) il passe. Fix pressenti : `ensureValidated` navigue explicitement vers l'étape Génération (le rail est libre pour un club existant) au lieu de dépendre de l'atterrissage guidé |
