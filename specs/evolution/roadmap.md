...
| P4-92 | **La CI survit aux pannes des sources tierces (Trivy, gitleaks)** | 🟡 | S | 3ᵉ épisode mesuré le 2026-08-13 : la base de vulnérabilités **Trivy** rendait 404 depuis `mirror.gcr.io` (après le 503 de **gitleaks** sur Docker Hub le 2026-08-11, lui-même après le **phpredis** réglé par PECL) — trois pannes de sources tierces à quelques jours d'écart, chacune capable de rougir un required check sans faute de notre code. Remèdes : **cacher la base Trivy dans le cache Actions** + chaîne de repli `--db-repository` **ghcr → ecr → gcr** ; **miroir gitleaks dans `ghcr.io/maratech-fr`**. Objectif : plus aucun gate de sécu ne dépend d'un unique hôte amont |
