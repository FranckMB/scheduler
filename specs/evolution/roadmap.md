...
| P2-2 | **Boucle d'ajustement — « corriger sur place »** | 🟠 | M | **CADRÉ ET VALIDÉ le 2026-08-11** (plan + contrarian-review, décisions fondateur figées — mémoire projet `programme-six-lots`). Deux lots : **F1** — colonne `lock_origin` (RESERVATION\|MANUAL\|UNKNOWN, backfill par jointure `Reservation`, indécidable→UNKNOWN) + wrap de créneau enrichi (infos, contraintes applicables par composition depuis `GET /api/constraints`, origine du verrou — aujourd'hui `ScheduleSlotTemplate` n'a AUCUN champ d'origine) ; **F2** — déplacer une équipe vers un créneau vide avec **verdict MOTEUR** (nouvel endpoint engine `/validate-assignments` mono-candidat, baseline non-HARD fixée via `add_fixed_slots`, `CONTRACT_VERSION` 2.3→2.4 — ⚠ la doc dit encore 2.2, rattraper dans la PR du bump), re-validation AU COMMIT du move, Mercure dans le scope, 409 pendant une génération (sonde `ClubGenerationLock` à créer), marqueur « modifié manuellement depuis la génération » (le score devient périmé). ⚠ JAMAIS le rail one-time existant : son check ignore capacité/fenêtres/repos (`ManualEditService:91-135`) — et `applyOneTimeUpdate` est mal nommé, il mute le template en permanence (dette relevée). ⚑ **Le mode « Échanger » (F3) est DIFFÉRÉ au premier retour terrain d'un club bêta** (verdict contrarian validé fondateur : il porte l'essentiel du coût — quota dédié, batch, capability — pour une valeur non prouvée ; s'il revient : assumptions CP-SAT, seuil UX 2-3 s, le verdict batch ALLUME et le commit RE-VALIDE). Axes §7.1 : planning lifecycle + constraint semantics → NR gatants proposés |
...
| P2-23 | **Export « ma semaine d'équipe » — matrice équipe × jour** | 🟡 | M | **Cadré et validé le 2026-08-11** (2 PR). UN geste d'export produit LES DEUX vues — la grille gestionnaire actuelle ET une matrice une-ligne-par-équipe (cellule = gymnase + heure ; 2 séances le même jour = cellule divisée, MÊME ligne ; jours vides vides), le gestionnaire distribue la page qu'il veut (aujourd'hui il envoie la vue gestionnaire à tout le monde). AUCUNE route nouvelle : PDF devient multi-pages (worker Puppeteer → mode multi-sections rétro-compatible — ⚠ il force aujourd'hui `pageRanges:'1'` + zoom global, `frontend/worker.js:41-54` ; si Chromium déçoit, le repli 2-fichiers change le SCHÉMA → STOP retour fondateur), XLSX gagne une 2ᵉ feuille « Par équipe ». 1 crédit inchangé, purge P4-52 inchangée. Décisions figées : PNG = page 1 seule · cellule SANS coach · lignes par rang (S→A→B→C→D, sous-titres, groupes jamais coupés en pagination paysage) · équipes sans séance = ligne présente vide · Sam/Dim seulement si occupés · **PAS de matrice en périmètre mono-gymnase** (une ligne tronquée se lirait comme la semaine complète — règle P4-62, le périmètre s'annonce) · mutualisation = entrée dupliquée par ligne, **P2-17 non préempté**. Ordre : PR XLSX d'abord (la matrice partagée s'y construit) |
| P2-24 | **Doléances coach — le formulaire public passe en étapes par équipe** | 🟡 | M | **Cadré et validé le 2026-08-11** (1 PR, frontend SEUL — le POST reste octet-identique, verrouillé par test). Intro (le pourquoi) → une étape PAR équipe → récap → validation, progression visible ; mobile d'abord. Décisions figées : envoi UNIQUE à la fin (pas de sauvegarde partielle serveur) · retour arrière + « Modifier » depuis le récap qui REVIENT au récap · « Rien à signaler » = une tape, rien n'est envoyé pour l'équipe · `respondedAt` enfin exploité à l'intro (« vous avez répondu le… » — le GET le livre, la page l'ignore aujourd'hui) · filet `sessionStorage` local purgé à l'envoi (un rechargement mobile ne perd plus tout) · **confirmation vide retenue** : un coach dont tout est déjà bon envoie `submissions: []` et passe ✓ répondu au lieu de rester « silencieux » dans le radar (le backend l'accepte déjà — `markResponded` inconditionnel, `PublicCoachWishController:216-222`). Clé de doléance INCHANGÉE (P4-26 maintenue), rail token intouché. Passe de design obligatoire (page publique) |
...
| P4-82 | **Console superadmin — un bouton « Offre » unique (A3, dernier volet du lot)** | 🟡 | M | A1 (badge d'offre EFFECTIVE) livré le 2026-08-11, A2 (renommage `mark-next-season-paid` + horloge démo) livré le même jour — voir état des lieux. **Reste A3** : picker + popup OBLIGATOIRE « saison encaissée » (current/next) qui pose `plan_id` ET avance `paid_season_year` (GREATEST monotone, horloge démo D6 déjà en place), modale 12→7 items. Décisions figées : rail SA4 = **schéma d'arguments FERMÉ** par action (enums, fail-closed 400 — la propriété écrite « jamais d'argument libre » devient « arguments bornés par schéma fermé », security-review dessus) · Découverte sans popup · Bêta AVEC popup (sinon elle naît expirée — le bug vécu) · catalogue des 6 offres CONSERVÉ. `SetClubPlanCommand` gagne `--paid-season` optionnel. Adjacent ouvert : P4-78 |
| P4-83 | **Zéro identifiant interne dans un texte lu par un humain** | ⚪ | S | **Cadré et validé le 2026-08-11** (1 PR backend, APRÈS P4-82 — le lot A réécrit certains textes). Inventaire exhaustif recoupé : **49 occurrences dans `/api/docs`** (8 identifiants — sources réelles = 14 chaînes de `CustomRoutesOpenApiFactory` + des docblocks d'ApiResource/Dto que API Platform SÉRIALISE dans le contrat : la réf migre en commentaire `//` non sérialisé, la substance reste) · la description du catalogue admin (`AdminActionCatalog:40`) · les descriptions CLI (`bin/console list` compte comme lu — décision fondateur, « SA4 » se purge aussi). **Garde** : `PublicTextIsFreeOfInternalIdentifiersTest` — balaye le document OpenAPI GÉNÉRÉ (immunisé contre « corrigé dans le snapshot, pas la source ») + catalogue + CLI, avec CONTRÔLE POSITIF embarqué (le pattern doit matcher « (SEC-07) » et « P1-5 » — leçon « un grep à zéro ne prouve rien »). Hors scope figé : commentaires de code (autorisés), `ImplicitConstraintConfig` (contrat machine byte-comparé avec l'engine, personne ne le lit) |
