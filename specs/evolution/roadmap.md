...
| P4-51 | **`max_days_per_week` coach — l'ÉCRAN manquant** | ⚪ | S | ⚠ **Prémisse corrigée le 2026-08-09 : la ligne disait « Vérifié : rien n'existe », c'était faux sur les trois zones.** **Moteur** : `max_days_override` est une contrainte HARD (`constraints.py:455+` — `is_working[coach, jour]` par réification, plafond sur lundi-vendredi) + diagnostic de surcharge (ENG-24). **Backend** : `Coach.maxDaysOverride` ET `maxDaysOverrideConfirmed` en base, DTO, processor, ressource API, recopiés à la transition de saison — et `ScheduleConstraintBuilder:697-698` les ENVOIE au moteur. **Frontend** : rien, zéro occurrence. ⚑ C'est le motif **SEC-13** : une capacité que le moteur honore et que le backend persiste, qu'**aucun geste UI ne peut créer**. `FACILITY_CAPACITY` a été retirée pour cela ; ici le besoin existe (fondateur : « je m'en fous des jours, mais être au club plus de 3 jours dans la semaine est contraignant » — un plafond de COMPTE, distinct de `COACH_AVAILABILITY` qui dit QUELS jours), donc la sortie est inverse : **on expose**. Reste à cadrer : le rôle de `maxDaysOverrideConfirmed` (posé par le processor, recopié en N+1) avant de dessiner le formulaire |
