...
| **Jours fériés au calendrier** (afficher les fériés dans le cockpit) | 🟡 | [API etalab jours-feries](https://calendrier.api.gouv.fr) | **Backend livré (`app:public-holidays:import` + `GET /api/public-holidays`, PR #63) — gradué dans [`../courantes/vacances-scolaires-jours-feries.md`](../courantes/vacances-scolaires-jours-feries.md)** (y compris hors périmètre assumé : Alsace-Moselle, 977/978, fallback offline). **Reste (⬜)** : rendu **cockpit frontend** (pastille) = PR suivante. Le pont reste une décision gestionnaire (`periodType=closure`) · **🟢** |
