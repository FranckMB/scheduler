...
| P5-6 | **Canal signalement + support/repro — cadrage ÉCRIT, décisions fondateur à trancher** | 🟡 | S/M | Besoin fondateur 2026-08-09. **Le cadrage est posé** : [`canal-signalement.md`](canal-signalement.md) — état des lieux vérifié (le snapshot rejouable existe déjà, Sentry câblé mais éteint/P5-1, zéro canal ni corrélation), 3 familles d'options (in-app minimal pressenti — le contexte auto-joint est la moitié de la valeur), **6 décisions D1-D6 en attente du fondateur**, anti-scope (pas de tickets/chat/upload). Implémentation après GO sur les décisions |
