...
| P5-10 | **Métriques de capacité — COLLECTE LIVRÉE (PR 1, 2026-08-13), reste la VUE superadmin (PR 2)** | 🟡 | S | La collecte tourne : 9 champs engine optionnels (contrat **2.6**), `queued_at`/`solve_started_at`/`payload_bytes` côté backend — chaque solve de prod est un point de mesure (trace : état des lieux). **Reste la PR 2** : `GET /api/admin/capacity` (patron `AdminMonitoringService`, fenêtre 90 j) + section « Capacité » dans l'onglet Infrastructure de la console SA — maquette validée au plan : volume/jour + profil horaire, attente p50/p95, durées par tranche de taille (% budget, % FEASIBLE), RAM vs 512 MiB, issues. `SuperAdminAccessTest` à étendre à la route (même PR). Le recalibrage des budgets 60/180/600 attend quelques semaines de données réelles — hors lot |
