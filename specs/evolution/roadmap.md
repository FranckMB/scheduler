...
| **Collecte des dispos coach par lien sans login** (questionnaire email → contraintes datées de période) | ⬜ | v3 §8.2 · FF#2 | **Reporté volontairement (décision utilisateur — idée validée, à ne pas perdre).** **Problème** : pendant une période d'exception les dispos saisonnières des coachs ne valent plus ; générer l'overlay sans les vraies dispos ⇒ plan injouable ; aujourd'hui le gestionnaire téléphone puis re-saisit à la main. **Mécanisme cible** : bouton « Solliciter les coachs » sur une période → email par coach avec **lien tokenisé sans login** (précédent technique : reset-password — token opaque, page publique, expiration) → mini-page publique « {fenêtre} — coche tes jours indisponibles + commentaire » → réponses persistées en **contraintes datées `COACH_AVAILABILITY`** (`calendarEntryId` de la période) → le wizard mode période les montre, la génération d'overlay les consomme (branchement palier B déjà en place) ; suivi « qui a répondu » + relance. **Utilité** : overlay directement jouable · travail distribué (le coach saisit SES dispos, friction zéro sans compte) · **différenciateur commercial** (cible mi-2027). **Ex.** : « Barros fermé sem. du 4 mai → 5 coachs sollicités → Cyril coche mar/jeu indispo (“dispo qu'après 18h le mercredi”) → l'overlay généré évite ces créneaux. » **Forme du questionnaire** (jours seuls vs grille jours×plages) à trancher à l'implémentation · **🟡** |
...
| **MAJ des dates de vacances côté superadmin** (nouvelle année scolaire 1×/an) | ⬜ | — | Aujourd'hui : re-run manuel de `app:school-holidays:seed` après édition du JSON versionné. **À faire** : UI/écran superadmin pour éditer/importer les périodes `school_holiday_period` sans redéploiement (upload JSON ou CRUD dédié, hors tenant — table globale). Rattaché à la future console superadmin · **🟡** |
| **Calendriers vacances DOM/TOM** (hors zones A/B/C métropole) | ⬜ | — | Chaque DOM (Guadeloupe, Martinique, Guyane, Réunion, Mayotte) et chaque TOM (Nouvelle-Calédonie, Polynésie, Wallis, St-Pierre-et-Miquelon…) a **son propre calendrier scolaire**, non dérivable de la zone A/B/C. Aujourd'hui : `SchoolZoneResolver` renvoie null (≥ dép. 96) → zone/vacances **à saisir à la main**. **À faire** : modèle « territoire » (au-delà de zone A/B/C) + saisie manuelle **ou import API** par territoire, alimentant `school_holiday_period`. Rattaché à la console superadmin ci-dessus · **🟡** |
