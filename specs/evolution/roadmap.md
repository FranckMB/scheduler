...
| **Console super-admin** (monitoring santé/usage/conversion + métriques par club & globales + data ops auto + actions d'exploitation) | 🟡 | idée produit (2026-07-09) | **SA0 backend + frontend livrés (2026-07-16)** → vérité courante [`../courantes/superadmin-auth.md`](../courantes/superadmin-auth.md), suite dans [`console-superadmin.md`](console-superadmin.md). Identité séparée + TOTP + firewall/session + CSRF + audit fail-closed + shell React `/admin` + NR phase1/frontend. Restent **SA1 (capture métriques) → SA2 (monitoring) → SA3 (jobs data) → SA4 (actions support) → SA5 (impersonation)**. **🔴** |
