...
| P2-28 | **Règles implicites : les nommer, régler leur intensité, toujours diagnostiquer** | 🟠 | L | Besoin fondateur 2026-08-14, né de la reproduction du planning réel BCCL (P5-13) : le layout réel 100 % verrouillé est INFEASIBLE parce que deux règles implicites du moteur sont plus strictes que la réalité — 3d « jamais 3 créneaux dos-à-dos, coaché ET joué » (`engine/app/solver/constraints.py:643` — Enzo coache U13F1 puis U18F1 puis JOUE avec SM1, même gymnase) et 3b « un jour de repos lun→ven, coaché + joué » (`constraints.py:452` — Emerick et Mathis présents les 5 soirs). Décisions figées : onglet dédié listant les **12 règles NOMMÉES** (extension de l'encart P4-55) en deux familles — **lois physiques immuables** (lecture seule) et **règles de bien-être réglables** (HARD / PRÉFÉRÉ / DÉSACTIVÉE, **seuils paramétrables** où pertinent : « 3 consécutifs » peut être 4) ; **défaut = comportement actuel** (tout HARD) ; **violation TOUJOURS diagnostiquée** quel que soit le cran (« Mathis est présent les 5 soirs »). Lot phasé : **PR 1 moteur + contrat 2.7 LIVRÉE le 2026-08-14** (trace état des lieux — 2 crans HARD/PREFERRED après contrarian-review, pénalités agrégées −6 prouvées, diagnostic `implicit_rule_not_honored`+`ruleKey`) · **PR 2 backend LIVRÉE le même jour** (entité+RLS, API résolue, payload, parité verdict réparée en route — trace état des lieux ; P5-13 débloqué : 2 PUT documentés posent PRÉFÉRÉ pour BCCL) · **reste PR 3 UI** (l'onglet + lien « assouplir cette règle » depuis le diagnostic). Axe *constraint semantics* plein → NR sémantique par règle + smoke |
