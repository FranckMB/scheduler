...
| **Transition de saison** (copie éditable des entrées N→N+1 · multi-saison simultané · bascule mi-juillet non destructive · revue guidée) | 🟡 | v3 §9.2 · FF#3 | **⭐ Voir [`transition-de-saison.md`](transition-de-saison.md).** **P1 livré (PR-1/PR-2, 2026-07-06)** : (PR-1) résolution multi-saison — `SeasonResolver` (courante dérivée du calendrier, pivot 15 juillet), `SeasonFilter`, `X-Season-Id` validé, `/api/me` `seasons[]`+gates par saison sélectionnée. (PR-2) `SeasonTransitionService` (copie des entrées N→N+1 avec remaps + `parent_*_id`/`parentConstraintId`, contraintes permanentes seules, rien de généré copié) + `POST /api/seasons/{id}/transition` + **sélecteur de saison frontend** (bascule = X-Season-Id, action « Préparer la saison suivante »). **Reste (⬜)** : **readonly serveur complet** (`SeasonAccessGuard`, 409) + **purge CLI** `app:seasons:purge` = **PR-3** ; **P2** = revue guidée + re-date événements + alertes d'anticipation. **🟡** (PR-3) |
