...
| P2-29 | **Cibler une contrainte sur PLUSIEURS tags (intersection) ou en EXCLURE un** | 🟡 | M | Né de la co-construction BCCL du 2026-08-15 : la règle « pas de week-end » visait « toutes les équipes » alors que la vraie règle est « pas d'équipe **en compétition** » (une équipe qui JOUE le week-end ne s'y entraîne pas ; Baby/Micro/Académies/Loisirs y sont légitimes) ; idem « adultes ≥ 18h50 » qui attrapait le Basket Santé (senior d'ÂGE mais loisir). Or `ScheduleConstraintBuilder:858-880` ne résout qu'**UN** `targetTag` : ni intersection (« SENIOR **et** COMPÉTITION »), ni exclusion (« toutes **sauf** loisir »). Contournement appliqué chez BCCL : **deux tags maison** `COMPETITION` (32 équipes) et `SENIOR_COMPETITION` (9) créés au seed — un gestionnaire lambda ne devinera jamais ce détour, et chaque combinaison future demande un nouveau tag. Deux pistes à cadrer : (a) `targetTags[]` + `excludeTags[]` dans le `config` (moteur + wizard + validateur — axe *constraint semantics*, la portée d'un tag est un step bloquant `TeamTagScopeTest`) ; (b) plus simple et complémentaire : **un tag SYSTÈME `COMPETITION` dérivé du `level`** (`TeamTagService::determineTagNames` sait déjà dériver NIVEAU/AGE/GENRE) — gratuit pour tous les clubs, mais ne résout pas l'intersection |
...
| # | Sujet | Impact | Effort | Note |
|---|-------|:---:|:---:|---|
| P5-14 | **Écrans système — le squelette d'une app en prod** | 🟡 | M | Besoin fondateur 2026-08-14. Existant : `ErrorBoundary` (crash JS → Sentry) + `RouteErrorBoundary` (chunk périmé post-deploy → « rechargez ») — `frontend/src/app/`. **Manquent** : (1) **404 route inconnue** — aucun catch-all `*` dans le router (`main.tsx`), une URL fausse rend un écran cassé ; (2) **500 / service indisponible** plein écran quand l'API ne répond pas au boot (le `me` initial échoue → il faut un écran, pas un spinner infini) ; (3) **page de MAINTENANCE** — statique, servie par l'edge (nginx/Caddy `error_page` 502/503) pendant deploy/migration : geste ops + un fichier HTML, PAS du SPA ; (4) **session expirée** — JWT mort → retour login avec message (« votre session a expiré »), jamais un 401 muet ; (5) **bannière hors-ligne** — perte réseau/SSE détectée, annoncée, reconnectée ; (6) `<noscript>` minimal. Chaque écran : sobre, ton de l'app, un seul geste proposé. À découper au cadrage (1+2+4 = front ; 3 = ops ; 5 = SSE existant à habiller) |
