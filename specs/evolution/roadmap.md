...
| P4-81 | **`test_no_venue_double_booking` rougit au hasard : il ignore la souveraineté du verrou HARD** | 🟠 | XS | Mesuré le 2026-08-11 : `Engine Tests` (required check) est tombé sur la PR #504, **qui ne touche pas l'engine**. Contre-exemple hypothesis rendu par la CI : un gymnase, UN créneau `capacity: 1` (jour 1, 17:00), deux équipes, et **deux `slotTemplates` `lockLevel: HARD`** épinglant chacune sur ce même créneau → `Venue double-booking at ('gym-a', 1, '17:00:00'): ['team-s', 'team-a']`. ⚑ **Le moteur a RAISON, c'est le test qui a tort** : un verrou HARD est pré-placé HORS du solveur (ALIGN-07, `constraint-matrix.md` §« Le verrou HARD est SOUVERAIN ») — la variable n'existe pas, `add_room_at_most_one` n'a rien à contraindre. Or l'assertion (`test_invariants.py:312`) est `len(team_ids) <= 1` **en dur** : elle ne lit ni la `capacity` du créneau, ni le `lockLevel`. Le générateur peut produire le cas parce que sa dédup porte sur `(teamId, venueId, jour, heure)` (`:194`) — deux ÉQUIPES différentes sur le même créneau passent. Intermittent : rouge seulement quand hypothesis tire cette forme, d'où un required check qui bloque des PR au hasard. ⚑ **Le patron du correctif existe déjà dans le même fichier** : `test_coach_consistency` exempte explicitement les créneaux verrouillés (`:349`). À faire : rendre l'assertion consciente de la `capacity` ET des verrous. ⚠ **Retombée P4-55** : l'encart annonce « un gymnase ne dépasse jamais sa capacité » — vrai du SOLVEUR, faux d'un co-épinglage HARD au-delà de la capacité. À arbitrer avec le correctif : nuancer l'énoncé, ou laisser (le gestionnaire qui sur-épingle sait ce qu'il fait, cf. la doctrine « il a le droit d'épingler, il a le droit de savoir »)|
