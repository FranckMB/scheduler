...
| P4-89 | **La CI meurt quand GitHub tousse : chaque build d'image retélécharge phpredis depuis codeload** | 🟡 | S | Mesuré le 2026-08-12 : **trois jobs tués le même jour** par un `curl: (22) … 503` sur `github.com/phpredis/phpredis/archive/6.3.0.tar.gz` — le Dockerfile php-fpm télécharge l'archive à CHAQUE build sans cache ni repli, donc un incident GitHub rougit `blocking-tests`/`phpstan`/`engine-semantics` sans qu'aucun code n'ait tort ; l'opérateur relance à la main et perd ~10 min par flake (précédent voisin : le `docker pull` de gitleaks, 503 le 2026-08-11). **Issues possibles** : (a) `pecl install redis` (miroir PECL, découplé de GitHub) ; (b) cache de build GitHub Actions (`type=gha`) pour que la couche survive entre runs ; (c) vendorer l'archive dans le dépôt (lourd). (a)+(b) se cumulent bien. ⚠ Le même Dockerfile sert le DEV local : la solution doit marcher hors CI aussi |
