...
| **Transition de saison — P2 (revue guidée + confort)** | ⬜ | v3 §9.2 · FF#3 | **Suite du P1 livré.** Contenu (spec [`transition-de-saison.md`](transition-de-saison.md) §5, P2) : (1) **wizard de revue guidée** par type de donnée à la transition — gymnases garder/retirer, coachs garder/parti/nouveau + réaffectation, équipes garder/dissoudre/créer + re-classement priorité, contraintes reconduites éditables (aujourd'hui la copie est muette puis on édite dans le wizard existant — P2 = l'écran de revue explicite keep/modify/dissolve). (2) **re-datation des événements club** (`CalendarEntry kind=event`) : reconduits mais « garder ? + nouvelle date » (P1 ne copie **pas** les CalendarEntry). (3) **alertes d'anticipation** mi-mai→mi-juillet (« prépare la saison suivante », J-x) — cron/radar, patron `app:periods:remind`. (4) à décider : report des **tags custom** entre saisons (bloqué par la dette `TeamTagService`, cf. ligne team_tags). **Dépend du P1 (livré).** · **🟡** |
...
| Import **calendrier de matchs FFBB** (`competitions`, `ffbb_club_code`) | ⬜ | v3 §1.4 · FF#19 | **♻ = l'entrée du module matchs — voir [`gestion-matchs-ffbb.md`](gestion-matchs-ffbb.md) §5.** Acté **V1** (pas V2) : la liste des rencontres vient de la FFBB (patron `FfbbExcelImporter` existant, spike sur le format d'export). Distinct de l'annuaire adverse (heures/positions, effet réseau) · **🟡** |
| **Planification des matchs** (placement domicile + radar de conflits + dérogations) | ⬜ | v3 §1.4 · FF#21 | **⭐ Besoin spécifié — voir [`gestion-matchs-ffbb.md`](gestion-matchs-ffbb.md).** Cadré au terrain : **module autonome** (gating découplé de l'entraînement), cœur = **placement manuel + radar de conflits + workflow dérogation**, **pas un solveur** (auto-place CP-SAT = assist V2). Conflit atomique = **dualité coach/joueur** (`COACH_(PLAYER_)NO_OVERLAP` réutilisés) + **dimension spatiale** (trajet siège↔ville, infra partagée avec FF#5). **Annuaire adverse global** (hors tenant, public-seulement, enrichi par l'usage — patron fériés/vacances + garde-fou sécu). Import FFBB liste des rencontres V1 (export FBI par équipe, patron `FfbbExcelImporter`) + ajout manuel + amicaux. **Catalogue-ligue seedé** (fenêtres HARD par catégorie×niveau, seed AURA capturé, hérité+éditable par le club — patron vacances/fériés). Coach/joueur branché sur `CoachPlayerMembership`. **Phasé A/B/C.** Le `🔴` monolithique se dissout en paliers · **🟡** |
