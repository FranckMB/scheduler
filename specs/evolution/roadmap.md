...
| P4-51 | **`max_days_per_week` coach — reste l'ÉCRAN** | ⚪ | S | **Le moteur applique désormais le plafond** (2026-08-09, arbitrage : PRÉFÉRÉ, jamais dur) : malus `overload_day` par jour au-delà de `maxDaysOverride`, borné à 15 < 21 pour ne jamais coûter une séance ; la garantie du jour de repos vaut pour TOUS (le skip « override ≤ 4 » est mort — il RETIRAIT le repos sans rien plafonner). **Reste** : l'écran (le champ n'est exposé nulle part — motif SEC-13) et le sort de `maxDaysOverrideConfirmed`, qui traverse tout le pipeline et **n'est lu par personne** : le supprimer, ou lui donner un sens |
