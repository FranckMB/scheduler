...
| # | Sujet | Impact | Effort | Note |
|---|-------|:---:|:---:|---|
| P4-94 | **Une contrainte HARD affichée ne nomme pas l'équipe concernée** | ⚪ | S | Retour fondateur 2026-08-14 (constaté en cliquant des contraintes HARD). Le wizard fabrique pourtant des noms « \<cible\> · \<effet\> » (`ConstraintsStep.tsx:211-266`), mais l'affichage planning met la **description dérivée en tête** et relègue le nom en seconde ligne **tronquée** (`SlotDetail.tsx:84-85`) — et `describeConstraint` ne nomme jamais la cible (elle décrit l'effet seul). Fix probable : la description dérivée porte aussi la cible (équipe/groupe/club). ⚠ **Écran exact du constat à préciser avec le fondateur avant de coder** (panneau de créneau ? récap ? diagnostics) — ne pas corriger un écran que le constat ne visait pas |
| P4-95 | **Atterrissage guidé : autofocus/scroll sur la contrainte à corriger** | ⚪ | S | Retour fondateur 2026-08-14. « Corriger cette contrainte » (P2-25, `SlotDetail.tsx:92-99`) ouvre l'éditeur wizard pré-rempli (`params={{ edit: c.id }}`) — l'attente exprimée : à l'arrivée, l'écran **scrolle et met en évidence** la contrainte visée au lieu de laisser l'utilisateur la chercher. À cadrer sur l'écran réel : vérifier ce que fait déjà l'atterrissage P2-25 (pré-rempli ≠ visible), et si le lien manque depuis d'autres surfaces (diagnostics de génération, erreurs du récap) |
