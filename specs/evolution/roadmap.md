...
| **Console super-admin** (monitoring santé/usage/conversion + métriques par club & globales + data ops auto + actions d'exploitation) | 🟡 | idée produit (2026-07-09) | **SA0 + SA1 + console read-only SA2 + socle jobs SA3-A + supervision jobs SA3-B livrés (2026-07-16)** → vérité courante [`../courantes/superadmin-auth.md`](../courantes/superadmin-auth.md), suite dans [`console-superadmin.md`](console-superadmin.md). Reste **solde SA3 (planification/relances) → SA4 → SA5**. **🔴** |
...
| P1-2 | **Console superadmin — SA0 à SA2 + jobs SA3-A/B** ✅ | 🟠 | L | crons vacances/purge · refresh FFBB manuel · actions support · impersonation lecture | Auth/MFA/audit, métriques, monitoring React, historique et supervision read-only des jobs livrés ; suite tracée §9 |
