...
| P4-93 | **Reprendre la PR #407 — la cellule « fenêtre coach » manque à la matrice sémantique** | 🟡 | S | La branche `test/coach-window-matrix` (GARDÉE sur décision fondateur 2026-08-14) contient un travail jamais livré : la preuve sémantique que la fenêtre horaire d'indisponibilité coach (`fromTime`/`untilTime`) est réellement appliquée par le solveur — cellule absente de `engine/tests/semantic/constraint_matrix.py` (vérifié), alors que l'infrastructure de matrice existe. Axe *constraint semantics* §7.1. ⚑ La PR porte aussi une leçon de méthode à conserver : son 1er jet était un FAUX VERT (test vert même fenêtre neutralisée — « bloquer plus » produit le même constat) ; une fenêtre ne se prouve que sur ce qu'elle LAISSE PASSER (créneau dedans refusé ET créneau dehors pris, même jour). Reprise à bas coût : rebase + re-run |
