...
| **MAJ automatique des vacances via l'API calendrier scolaire** (1×/saison) | ✅ | [API data.gouv](https://www.data.gouv.fr/dataservices/api-calendrier-scolaire) | **Livré (commande `app:school-holidays:import`)** : enrichit `school_holiday_period` depuis l'**API officielle Éducation nationale** (ODS `data.education.gouv.fr/api/explore/v2.1`, dataset `fr-en-calendrier-scolaire`), pagination, upsert idempotent `(zone, holidayType, schoolYear)` comme le seed. Fenêtre = saison courante + N+1. **Filtre corrigé au terrain** : `population="-"` (vacances de trimestre) **or** `"Élèves"` (été côté élèves, l'été « Enseignants » est écarté) — le `population=Élèves` seul ne rendait que l'été. **Règle vacance** : > 3 jours ouvrés (lun–ven) **et** libellé ≠ `Pont …` → écarte ponts (Ascension), semaines courtes, marqueurs 1 jour. `holidayType` = **slug ouvert** dérivé du libellé (`toussaint`…`carnaval`/`ete_austral`/`apres_1ere_periode`) pour couvrir métropole + DOM/TOM. Le JSON versionné + `app:school-holidays:seed` restent le fallback hors-ligne. **Différé** : cron annuel + bouton superadmin (rattachés à la future console superadmin) · **🟡** |
| **Calendriers vacances DOM/TOM** (hors zones A/B/C métropole) | ✅ | — | **Livré avec l'import ci-dessus.** Le modèle « territoire » = **13 codes de zone** (`SchoolZoneResolver::ZONES` : A/B/C + CORSE + 9 DOM/TOM), `SchoolZoneResolver` mappe Corse `2A/2B/20 → CORSE` et dép. 971–988 → territoire (backfill migration re-tag les clubs corses `B → CORSE`). L'import remplit `school_holiday_period` avec les calendriers **spécifiques** (vérifié en réel : Antilles/Guyane `carnaval`+`paques`, Réunion `ete_austral`+`apres_Nème_periode` austral, Polynésie `septembre/novembre/fevrier/mai`). Territoires non encore publiés côté API (ex. Nouvelle-Calédonie, Wallis certaines années) → saisie manuelle en attendant · **🟡** |
| **Jours fériés au calendrier** (afficher les fériés dans le cockpit) | 🟡 | [API etalab jours-feries](https://calendrier.api.gouv.fr) | **Backend livré (commande `app:public-holidays:import`)** : table globale `public_holiday` (no RLS) remplie depuis l'API etalab `calendrier.api.gouv.fr/jours-feries/{zone}.json` (10 fichiers : metropole + 9 DOM/TOM). Modèle = `zone='NATIONAL'` (métropole, tous les clubs) + extras territoriaux = **diff territoire − métropole** tagués sur les codes de `SchoolZoneResolver::ZONES` (ex. Abolition de l'esclavage → GUADELOUPE/RÉUNION…, à leur date). Fenêtre = année civile courante + N+1, upsert idempotent `(zone, date)`. Lu via `GET /api/public-holidays` (NATIONAL ∪ `Club.schoolZone`, zone null → NATIONAL quand même), flag `national` par item. **Display-only** — aucun impact solveur (si un férié gêne un entraînement, le gestionnaire pose une fermeture/planning exceptionnel). **Reste (⬜)** : rendu **cockpit frontend** (pastille) = PR suivante. **Hors périmètre assumé** : Alsace-Moselle (Vendredi Saint/26 déc — dép. en zone B sans zone dédiée), Saint-Barthélemy/Saint-Martin (977/978 hors `ZONES`), fallback JSON offline (parité avec le seed vacances, non construit). Le pont reste une décision gestionnaire (`periodType=closure`) · **🟡** |
