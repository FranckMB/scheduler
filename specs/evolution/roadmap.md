...
| SEC-16 | **Messenger : le transport d'échec PARTAGE le stream Redis des messages** | 🟠 | S | **Découvert 2026-08-07 en testant SEC-15** : `MESSENGER_FAILURE_TRANSPORT_DSN=redis://redis:6379/messages/failed` = MÊME stream `messages`, groupe `failed`. Dès que ce groupe est MATÉRIALISÉ (un `messenger:setup-transports`, ou possiblement le premier message réellement routé en échec), TOUT dispatch rend 500 : le transport redis refuse `delete_after_ack` quand plus d'un groupe existe sur le stream (« it risks deleting messages before all groups could consume them »). Reproduit puis réparé à la main (`XGROUP DESTROY messages failed` — groupe vide). Fix : stream DÉDIÉ pour l'échec (`redis://redis:6379/failed_messages/failed`) + vérifier la migration des éventuels messages en échec au moment du changement. ⚠ En prod, la première panne réelle d'un handler pourrait geler tous les dispatchs — prioriser avant la commercialisation |
