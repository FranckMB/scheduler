...
| Salles **divisibles** + `max_parallel_trainings` (FACILITY_MAX_PARALLEL_TRAININGS) | 🟡 | contraintes-v2 | Livré : case « Terrain divisible » (`canSplit`) + capacité 1/2 par créneau, **garde-fou** builder (S4 : salle non divisible ⇒ capacité créneau forcée à 1). **Manque** : `max_parallel_trainings` > 2, contrainte dédiée |
...
| Objectif mou : **regroupement même-coach-même-salle** (+50) | ⬜ | v3 §4.3 | Poids `grouping` était **mort** (défini, jamais câblé) → retiré (S1) ; à ré-implémenter si souhaité |
| Niveau 0 — extraction des locks HARD hors solveur | 🟡 | v3 §4.1 | Locks HARD gérés ; formaliser le pré-placement |
| INFEASIBLE D1–D4 : rapport dégradé, diagnostics de conflit, suggestions texte, **relaxation partielle (D4 = P2)** | 🟡 | v3 §7 | Diagnostics **précis** livrés (S5 : qui/quand/pourquoi — équipes/salle/jour+heure nommés, raison du non-placement, indice de sous-capacité INFEASIBLE) ; D4 (relaxation auto) non |
