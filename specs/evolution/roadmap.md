...
| P4-84 | **Les deux vues équipe × jour ne trient pas pareil** | ⚪ | S | Signalé par le coder en livrant D2 (2026-08-11), confirmé au code : le **XLSX** trie ses lignes par **catégorie puis nom** (`SpreadsheetGenerator::appendTeamDayMatrix`, `uasort` sur `[category, name]`) tandis que le **PDF** les groupe par **rang** S→A→B→C→D puis `tierOrder` puis nom (`PdfGenerator::buildMatrixSection`) — décision figée de P2-23 pour le PDF, parce que le rang porte les sous-titres et la pagination par groupes entiers. **Un gestionnaire qui ouvre les deux exports des MÊMES données voit deux ordres de lignes.** Ce n'est pas un bug (chaque tri est correct chez lui) mais une incohérence produit. **Trois issues, décision fondateur** : (a) aligner le XLSX sur le rang — cohérent, mais le XLSX perd le tri par catégorie qui sert au filtrage Excel ; (b) assumer la divergence et la documenter (le PDF est un document à distribuer, le XLSX un tableau à trier soi-même) ; (c) donner le rang EN COLONNE dans le XLSX pour que l'utilisateur trie comme il veut. ⚠ Ne rien faire par défaut : les deux vues sont nées le même jour, c'est le moment de trancher pendant que le contexte est frais |
| P4-85 | **Trois constantes de version, quatre nombres, et les docs les confondent** | ⚪ | S | Relevé en bumpant le contrat en F2a (2026-08-12). Coexistent : `engine/CONTRACT_VERSION` = **2.4** (le contrat backend⇄engine), `ScheduleConstraintBuilder::SCHEMA_VERSION` = **2.2** et `MatchPlacementPayloadBuilder::SCHEMA_VERSION` = **2.2** (la version que le PAYLOAD s'attribue), `ImplicitConstraintConfig::RULESET_VERSION` = **2.3** (le jeu de règles implicites). **Ça FONCTIONNE** — l'engine ne compare que le MAJOR (`main.py:527,545,569` : `version.split('.')[0]`), donc 2.2 face à 2.4 passe — mais un lecteur ne peut pas savoir lesquels doivent bouger ensemble, et les docs emploient « version du contrat » pour les trois. Le risque n'est pas la panne, c'est le **bump manqué le jour où il compte** : un changement de forme du payload sans bump de MAJOR passerait inaperçu des deux côtés. **À faire** : nommer les trois concepts distinctement (contrat / payload / ruleset) là où les docs les confondent, et décider si les `SCHEMA_VERSION` doivent suivre `CONTRACT_VERSION` ou vivre leur vie — **puis le garder par un test**, sinon la dérive revient. ⚠ Ne PAS aligner mécaniquement les nombres avant d'avoir tranché le sens : ça masquerait la question |
...
| P5-7 | **Policies RLS « rôle admin » — LE prérequis du PostgreSQL managé** | 🟠 | S/M | Découvert en chiffrant Scaleway (2026-08-11, `business/hebergement/scaleway-mise-en-prod.md`) : **un Postgres managé ne donne JAMAIS `BYPASSRLS`** (pas de superuser — vérifié Scaleway, feature request ouverte chez eux ; même contrainte chez tout provider). Or notre connexion `admin` bypasse la RLS PARCE QU'elle est superuser, et les tables sont en `FORCE ROW LEVEL SECURITY` (le propriétaire ne bypasse pas) : sur un managé, la console superadmin, les migrations de données inter-clubs et tous les scripts « porte admin » se font bloquer fail-closed. **La sortie propre** : des policies CIBLÉES PAR RÔLE — `CREATE POLICY admin_all ON <table> TO <rôle admin> USING (true) WITH CHECK (true)` sur chaque table RLS (le propriétaire peut les créer sans superuser). Une migration + extension de `RlsIsolationTest` (le rôle admin voit tout ET `app_user` reste isolé — falsifier les deux sens). ⚠ À livrer AVANT toute migration vers un managé, chez n'importe quel provider — sans ce lot, l'option B de l'étude d'hébergement est morte. Axe tenant isolation §7.1 → NR (le test étendu EST le NR, déjà step du gate) |
