...
| AUD-ENG-28 | Les invariants tournent sur un pipeline qui n'est pas celui de prod | Faible | engine | 2026-08-08 | `tests/invariants/test_invariants.py:31-36` : `_run_pipeline` local, sans `parse_v2_constraints`, coach lu des `slotTemplates`. « Deferred to E1 » assumé dans le code. Trou limité — le golden BCCL passe, lui, par le vrai `build_schedule` |
| AUD-ENG-29 | Le harnais de test émet un `config` que l'API refuserait | Faible | engine | 2026-08-08 | `tests/support/pipeline.py:106` injecte `config["coachId"]`, clé supprimée par SEC-13 (doublon du scope) : le harnais « contract-accurate » produit un payload qui prendrait 422. Zéro effet solveur — dérive de fidélité |
| AUD-ENG-30 | `/place-matches` (synchrone) attend derrière un solve de saison | Faible | engine | 2026-08-08 | Sémaphore global partagé (`main.py:534`, `max_concurrent_solves=1`) : le rail matchs est **synchrone** et peut attendre ~600 s → timeout HTTP côté gestionnaire. Tradeoff documenté (« still bounds CPU »). Devient réel dès que deux clubs travaillent en même temps |
| AUD-ENG-31 | Chemins morts dans le parseur de contraintes | Mineure | engine | 2026-08-08 | `parsed["fixed_slots"]` n'est jamais alimenté (`constraints.py:1709`) mais reste câblé (`main.py:375`) ; vestiges du two-pass abandonné (`skip_rest_day_and_distribution`, ADR-0001). Hygiène : du code qui suggère un mécanisme inexistant |
