...
| **Périodes d'exception** (`period_templates` + `is_cutoff`, `period_template_slots` avec `team_ids[]`, `period_assignments`, `period_coach_responses`) | ⬜ | v3 §3.6, §8 · FF#2 | **♻ = `CalendarEntry` `kind=period`** (cockpit §4, §9ter) : `periodType` (closure/holiday/cutoff/mutualisation) + contraintes datées (`Constraint.calendarEntryId`) + overlay (`Schedule`). **1 entité au lieu de 4 tables** · **🟡** (palier B) |
| Scheduler quotidien @8h : détecte période sous 14j → assignment pending + alerte (J-7 rappel, J-3 rouge), jamais d'auto-action | ⬜ | v3 §8.2 · FF#2 | **♻ = le panneau radar** du cockpit (cockpit §5). Version manuelle livrable au palier A ; cron au palier C · **🟡** |
