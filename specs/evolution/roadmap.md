...
| P4-86 | **`manual-edit/one-time` : route orpheline, et sa dette de nom** | ⚪ | S | Depuis F2b (2026-08-12) le déplacement passe par `/api/schedule-slots/{id}/move` (verdict moteur) : **plus aucun appelant frontend** de `manual-edit/one-time`, seuls des tests backend l'exercent. Elle n'a **pas** été supprimée dans F2b — retirer une route est un changement d'API, donc une décision fondateur, pas un geste au passage. Deux dettes à solder ensemble le jour où on tranche : (1) `applyOneTimeUpdate` **mute le template en PERMANENCE** malgré son nom — un lecteur croit modifier une occurrence et modifie la règle ; (2) son contrôle **ignore capacité, fenêtres et repos** (`ManualEditService:91-135`), c'est-à-dire exactement le danger que F2b vient de retirer du chemin du gestionnaire. **Trois issues** : (a) supprimer la route et ses tests — le plus propre si rien ne la réclame ; (b) la garder mais la faire passer sous le même verdict que `/move` ; (c) la garder telle quelle en documentant qu'elle est un outil interne non exposé. ⚠ Ne pas la laisser en l'état SANS décision : une route non appelée mais dangereuse est une porte ouverte qu'on redécouvre au pire moment |
