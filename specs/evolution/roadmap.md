...
| **Planification des matchs** (placement domicile + radar de conflits + dérogations) | ⬜ | v3 §1.4 · FF#21 | **⭐ Besoin spécifié — voir [`gestion-matchs-ffbb.md`](gestion-matchs-ffbb.md).** Cadré au terrain : **module autonome** (gating découplé de l'entraînement), cœur = **placement manuel + radar de conflits + workflow dérogation**, **pas un solveur** (auto-place CP-SAT = assist V2). Conflit atomique = **dualité coach/joueur** (`COACH_(PLAYER_)NO_OVERLAP` réutilisés) + **dimension spatiale** (trajet siège↔ville, infra partagée avec FF#5). **Annuaire adverse global** (hors tenant, public-seulement, enrichi par l'usage — patron fériés/vacances + garde-fou sécu). Import FFBB liste des rencontres V1 (patron `FfbbExcelImporter`) + ajout manuel + amicaux. **Phasé A/B/C.** Le `🔴` monolithique se dissout en paliers · **🟡** |
