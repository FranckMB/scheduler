...
> **Statut** : **SA0 à SA3 livrés** (2026-07-16) + **SA2-stats** et **SA4 v1 (catalogue d'actions)** livrés (2026-07-18) + **console en onglets + monitoring conteneurs/dépendances + journaux** livrés (2026-07-25) — vérité courante dans [`../courantes/superadmin-auth.md`](../courantes/superadmin-auth.md). Suite : SA4 v2 (suspension/approbation, au premier cas réel) → SA5.
>
> **Livré 2026-07-25 — console en onglets + monitoring** : la page `/admin` passe d'un scroll unique à **6 onglets** (Vue d'ensemble, Infrastructure, Jobs, Référentiels, Comptes clubs, Journaux) avec deep-link `?tab=`/`&sub=`, back-button, localStorage et pattern WAI-ARIA tablist. `GET /api/admin/health` étendu **append-only** : `containers[]` (statut + heartbeat par service ; la sonde mailpit n'est émise qu'en dev, la prod n'embarque pas ce conteneur) et `externalDependencies[]` (FFBB Meilisearch, ODS, etalab — GET timeout 2 s, jamais `api.ffbb.com` authentifié). Heartbeats ajoutés : `CronRunnerHealthSubscriber` (cache, TTL 180 s) + `frontend/worker.js` (pdf-worker, RESP minimal builtins-only, TTL 60 s). 3 journaux read-only SuperAdmin paginés : `GET /api/admin/audit-log`, `/api/admin/messenger/failed` (jamais le body — PII), `/api/admin/system-errors` (union job-runs échoués + audit erreurs, dédup). **Restart conteneur DIFFÉRÉ** : le bouton « Redémarrer » + le chemin `docker.sock`-dans-php-fpm ont été **retirés** — QA réelle a montré qu'ils ne fonctionnaient pas (plugin compose absent, socket non accessible au user fpm, nom de projet) et le socle `docker.sock` dans le conteneur web est un risque RCE→hôte qui ne se transpose pas en prod Scaleway. À reprendre comme feature dédiée contre l'orchestrateur prod réel (voir roadmap). L'abstraction n'est pas conservée (recréée le moment venu).
...
> **Rattachement roadmap** : **P4-54** (SA4 v2 → SA5) — s'appuie sur `solver_metrics` (§9, à persister) + audit trail (§9) + rétention/purge (§3).
