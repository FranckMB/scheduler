...
| DOC-14 | `../../backend/docs/backend-inventory.md:120` ment sur `/api/register` (201+JWT depuis #153 = 202 générique + `/api/register/verify` absent) ; ignore aussi Reservation (#132), gate `/api/club_users` (#147), GenerationComplexityGuard (#156). | doc | Élevée | confirmé code-lu | nouveau |
| DOC-15 | `../../frontend/docs/frontend-spec.md:450` même mensonge register (« 201… Stocker token ») + omet `/verify-email/:token` ; stamp 07-03, ~70 PRs de retard. | doc | Élevée | confirmé code-lu | nouveau |
