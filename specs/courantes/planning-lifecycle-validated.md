...
Last verified @ 2026-08-20 (**symétrie Valider/Rouvrir** — re-vérifié contre `PlanningToolbar.tsx` et `PlanningPage.tsx` : Rouvrir borné à `!embedded` (a quitté la toolbar embarquée du wizard), badge de statut visible dans les deux modes, `validate()` navigue vers `/planning` en succès — §4 recalé) — *(historique des passes retiré le 2026-08-19, audit DOC-33 : 3 entrées empilées. Un stamp REMPLACE, il ne s'empile pas ; l'historique vit dans git : `git log -p --follow specs/courantes/planning-lifecycle-validated.md`)*
