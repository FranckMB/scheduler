...
Last verified @ 2026-08-19 (**rotation de fraîcheur** — re-vérifié contre le code : `SocleGuard::assertSeasonPlanChosen` existe (`backend/src/Service/SocleGuard.php:26`), `Fixture.status` part bien de `UNPLACED` (`Entity/Fixture.php:70-71`), `Competition` est `TenantOwnedInterface`, `engine/CONTRACT_VERSION` vaut **2.12** et reste le fichier unique lu par `engine/app/main.py`, et le rail matchs garde son verrou DÉDIÉ distinct de celui de génération (`backend/src/Service/MatchPlacementLock.php`, ADR-0003) — rien de faux) — *(historique des passes retiré : un stamp REMPLACE, il ne s'empile pas ; l'historique vit dans git : `git log -p --follow specs/courantes/module-matchs.md`)*
