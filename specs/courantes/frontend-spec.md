...
- **Déplacer — mode cible click-click (P2-30 PR B, 2026-08-16, lot SOLDÉ)** : « Déplacer » sur le
  ...
  - **Case occupée → éviction, remplie par un ESSAI (P2-32, décision fermée D6+D8, 2026-08-16)** :
    ...
    **dry-run** — `POST /api/schedule-slots/{id}/move` avec `evictSlotId` = l'occupant **et
    ...
    (`frontend/src/features/planning/EvictConfirmDialog.tsx`) porte trois états :
    ...
    - Un dry-run qui échoue au TRANSPORT (verrou `target_locked` posé entre-temps, génération en
      cours 409, réseau) ferme la modale (`onError`, pas `onSuccess`) et toaste le motif — le
      mode cible reste armé.
...
- **Le hook POSSÈDE son feedback métier** (`useMoveSlot`/`usePlaceSlot`/**`useMoveDryRun`**) : un
  ...
  `SlotEditError`/`GenerationInProgressError` — la page les affiche dans son contexte) et ne parle
  que d'un vrai échec transport. Sans lui, le filet global `MutationCache.onError` (qui ne toaste
  QUE les mutations SANS `onError` de niveau hook) doublait un refus 422 en **« Problème de
  connexion. Vérifiez votre réseau. »** — mensonger, le réseau allait bien. `useMoveDryRun`
  (P2-32) suit la MÊME règle mais un dry-run **refusé n'est jamais une erreur** — c'est un 200
  `{valid:false}` qui **résout** (`onSuccess`), pas une exception ; son `onError` ne voit donc que
  `TargetLockedError`/`SlotEditError`/`GenerationInProgressError` (transport/concurrence), jamais
