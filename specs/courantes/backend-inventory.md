...
| 11bis | SchedulePlan | `/api/schedule_plans` | Conteneur nommé des versions d'une saison/période (lecture seule ; ADR-0002) — filtres `calendarEntryId`, `type`. Le renommage arrive avec la bascule (le nom vit alors sur le plan, inv. 12). Champ **dérivé** `hasVersions` (batché par le provider, patron `Team.isEngaged`) — ⚠ distinct de `chosenScheduleId` : un plan peut porter des versions sans qu'aucune soit pointée | |
