...
Last verified @ 2026-08-18 (re-vérifié contre `backend/src/Deletion/SlotPinStep.php` + `SlotDeletionTarget.php` + `backend/src/Service/EntityCascadeDeleter.php` — la cascade d'un CRÉNEAU entre au plan déclaratif (triplet + couche) et gagne sa route `deletion-impact` — §tableau des routes custom) ; précédemment : Last verified @ 2026-08-18 (re-vérifié contre `backend/src/Seed/BcclSeeder.php` + `backend/src/Seed/BcclSeedProfile.php` — recalage du seed BCCL sur la réalité du club (coachs complétés/ajoutés, 2 contraintes de jour, 4 préférences de gymnase) : la liste `FICTIONAL_COACHES` a grandi d'autant, la phrase ci-dessous ne porte plus de compte en dur — narrative complète : `etat-des-lieux.md`) ; précédemment : Last verified @ 2026-08-18 (re-vérifié contre `backend/src/Deletion/*` + `backend/src/Controller/DeletionImpactController.php` + `backend/src/Service/EntityCascadeDeleter.php` — **P3-16 + DOC-2, lot SOLDÉ** : la cascade de suppression d'une salle/équipe/coach devient un PLAN déclaratif (`CascadePlan`) exécuté par le deleter ET compté par `DeletionImpactCounter`, exposé par trois routes de lecture `deletion-impact` — §tableau des routes custom) ; précédemment : Last verified @ 2026-08-18 (re-vérifié contre `backend/src/Service/PeriodWindowUniquenessGuard.php` + `backend/src/Exception/WindowAlreadyPlannedException.php` + `backend/src/EventListener/WindowAlreadyPlannedListener.php` + `backend/src/State/Processor/SchedulePlanStateProcessor.php` + `backend/src/State/Processor/CalendarEntryStateProcessor.php` — **P2-38 PR2 backend** : ADR-0002 inv. 4 (« deux plans ne se chevauchent jamais ») tenu enfin par le code À LA NAISSANCE d'un plan de période — `PeriodWindowUniquenessGuard::assertWindowFree`, appelée aux deux seuls sites de naissance (POST `schedule_plans` = « Adapter », POST `calendar-entries` d'une semaine-enfant qui naît avec son plan), refuse en 409 `window_already_planned` dans les deux sens tout chevauchement avec un AUTRE plan de période, en NOMMANT le plan déjà en place (titre + fenêtre) ; le 409 structuré est composé par un listener `kernel.exception` (un State Processor seul ne rend que `{detail, status}`) ; la famille (ancêtre racine `COALESCE(parent_entry_id, id)`) reste exclue — §lignes `SchedulePlan`/`CalendarEntry`. Rien n'est supprimé ni rétréci automatiquement ; le FAIT (déclarer une entrée racine) reste libre. **PR3 front livrée le même jour (`frontend/src/features/cockpit/`) — P2-38 est LOT SOLDÉ (3 PR), l'item a quitté la roadmap ; détail écran : `accueil-cockpit-temporel.md` §5bis, trace : `etat-des-lieux.md`**) ; précédemment : 2026-08-18 (re-vérifié contre `backend/src/Service/PlanVenueClosures.php` + `backend/src/Service/VenueClosureDays.php` + `backend/src/Service/ScheduleConstraintBuilder.php` + `backend/src/Repository/ConstraintRepository.php` + `backend/src/Controller/CalendarEntryConflictsController.php` — **P2-38 PR1 backend** : une fermeture de gymnase devient un fait TRANSVERSAL — `PlanVenueClosures` lit désormais TOUTES les fermetures datées (`venue_closed`) du club+saison (`ConstraintRepository::findDatedFacilityByClubSeason`), chacune bornée à la fenêtre de SA PROPRE entrée porteuse (repli legacy) puis croisée avec la fenêtre du plan construit — au lieu des seules datées de SA PROPRE entrée (ou de sa mère) ; `ScheduleConstraintBuilder::buildForPeriodPlan` et `CalendarEntryConflictsController` dérivent désormais leurs jours fermés/`fullyClosedVenueIds` de cette maison unique plutôt que de leur propre sélection de période — §route `generate`/route `conflicts`/`ScheduleConstraintBuilder` ; PR2 (garde « une seule planification par fenêtre ») livrée le même jour, PR3 front décrite ci-dessus) ; précédemment : 2026-08-18 (re-vérifié contre `backend/src/Service/PlanVenueClosures.php` + `backend/src/Service/VenueClosureDays.php` + `backend/src/Service/OrphanPinGuard.php` + `backend/src/State/Processor/ReservationStateProcessor.php` + `backend/src/State/Processor/VenuePeriodOverrideStateProcessor.php` + `backend/src/Controller/CalendarEntryConflictsController.php` — **P2-37 PR1 backend + miroir** : gymnase entièrement fermé (indisponibilité DÉRIVÉE des fermetures datées) exclu de l'orphelinage comme un désactivé, réactivation refusée en 422 (POST/PUT/DELETE `VenuePeriodOverride`), réservation refusée en 422 sur gymnase/jour fermé, message `OrphanPinGuard` nommant l'équipe, `fullyClosedVenueIds` sur la route `conflicts` — §lignes `Reservation`/`VenuePeriodOverride`/route `generate`/route `conflicts` ; PR2 front (UI période + récapitulatif) reste ouverte, roadmap) ; précédemment : 2026-08-18 (re-vérifié contre `backend/src/Entity/ImplicitRuleSetting.php` + `backend/src/Repository/ImplicitRuleSettingRepository.php` + `backend/src/State/{Provider,Processor}/ImplicitRuleSettingState{Provider,Processor}.php` + `backend/src/Service/SchedulePlanProvisioner.php` — **bien-être PAR PÉRIODE, PR1 backend** : `ImplicitRuleSetting` gagne `schedulePlanId` (ADR-0002 inv. 5), copie matérialisée à la naissance du plan, DELETE en portée plan re-copie la saison — la ressource, absente de ce tableau jusqu'ici, y entre § tableau des ressources ; **PR2 front livrée le même jour** (l'onglet Bien-être exerce désormais la portée par plan — comportement décrit dans `frontend-wizard.md`, la note frontale retirée d'ici) ; précédemment : 2026-08-17 (re-vérifié contre `backend/config/packages/api_platform.yaml` + `App\Service\ProductIdentity` — **P5-15** : le titre OpenAPI reste littéralement `ClubScheduler API`, un résidu hors du périmètre de `ProductIdentityTest` (config/, pas src/) — noté ci-dessous, reste ouvert en roadmap) ; précédemment : 2026-08-17 (re-vérifié contre `backend/src/Service/MoveSlotService.php` + `backend/src/Exception/EngineTimeoutException.php` + `backend/src/Controller/ManualEditController.php` — **correctif d'incident terrain, pas un item de roadmap** : `VALIDATE_HTTP_TIMEOUT_SECONDS` 8 s → **20 s** (maison unique `move()`/`place()`, calé sur mesure) et le service distingue désormais le moteur TROP LENT (`EngineTimeoutException`, timeout traduit depuis `TimeoutExceptionInterface` → **504** `code=engine_timeout`) du moteur injoignable/cassé (502 inchangé) — §route `move`/`place-slot` ; l'incident : un déplacement LÉGAL rendait 502 car le backend abandonnait ~0,7 s avant la réponse du moteur ; trace datée : `etat-des-lieux.md`) ; précédemment : 2026-08-17 (re-vérifié contre `backend/src/ApiResource/SharedTrainingGroupResource.php` + `backend/src/State/{Provider,Processor}/SharedTrainingGroupState{Provider,Processor}.php` + `backend/src/Dto/SharedTrainingGroupInput.php` : **P2-27 PR A, mutualisation** — nouvelle ressource `SharedTrainingGroup` (`/api/shared_training_groups`) — §ligne ci-dessous) ; précédemment : 2026-08-17 (re-vérifié contre `backend/src/Controller/VenueUsageStatsController.php` + `backend/src/Service/VenueUsage/VenueUsageCalculator.php` + `backend/src/Enum/TeamLevel.php` : **P3-22, lot SOLDÉ** — nouvelle route `GET /api/venue-usage-stats` (stats d'utilisation des gymnases par jour, réelles vs à venir, ventilées par gymnase ET par niveau, calcul jour par jour sur `[from,to]`, résolution période validée → vacances (0 h des deux côtés, y compris Réalisé) → grille de saison pointée — §nouvelle section « Statistiques d'utilisation des gymnases ») ; précédemment : 2026-08-17 (re-vérifié contre `backend/src/Message/GenerateScheduleMessage.php` + `backend/src/Controller/RegenerateController.php` + `backend/src/MessageHandler/GenerateScheduleHandler.php` + `backend/src/Service/ScheduleConstraintBuilder.php` : P3-21 PR B, LOT SOLDÉ — le message de génération porte `sourceScheduleId` (version REGARDÉE par `RegenerateController`, pas la dernière du plan), le handler résout le précédent (source explicite de même lignée, sinon repli dernière `COMPLETED` du plan, sinon rien) et l'injecte via `withPreviousAssignments` **après** le hash de snapshot — le précédent est une préférence de convergence, jamais une donnée de structure, sous peine de dé-griser en permanence « Régénérer » — §route `regenerate` et sa note) ; précédemment : 2026-08-16 (re-vérifié contre `backend/src/Controller/ManualEditController.php` + `backend/src/Service/MoveSlotService.php` : P2-32 PR A — contrat 2.10, `MoveSlotService::move()`/`place()` gagnent un paramètre `dryRun` (essai : même chemin jusqu'au verdict INCLUS, gardes pré-moteur et verrou souverain compris, retour AVANT toute écriture — `ManualEditController` répond alors **200** `{valid, dryRun:true, violations, compromises, evicted?}` **même quand `valid=false`**, jamais 422 : un essai ne peut pas échouer au sens HTTP, il RAPPORTE) et les réponses ÉCRITES d'un candidat accepté portent désormais `compromises` (delta de confort nommé du moteur, P2-32 — voir `engine-inventory.md` §POST /validate-assignments) ; timeout **HTTP** de l'appel engine 2 s → **8 s** (`VALIDATE_HTTP_TIMEOUT_SECONDS`, un candidat accepté déclenchant jusqu'à 3 solves moteur — le budget **solveur** par solve reste 2 s) — §route `move`/`place-slot`) ; précédemment : 2026-08-16 (re-vérifié contre `backend/src/Controller/ManualEditController.php` + `backend/src/Service/MoveSlotService.php` : P2-30 PR A — `/move` gagne l'éviction optionnelle (`evictSlotId`, D3 verrou souverain, bloc `evicted`) et le nouveau path `POST /api/schedules/{id}/place-slot` crée une séance à la dérive sous verdict moteur, durée résolue **côté serveur** depuis la fenêtre de gymnase (`durationMinutes` du client = simple assertion, 422 `duration_mismatch`/`slot_unavailable` sinon) — §route `move`/`place-slot`) ; précédemment : 2026-08-16 (re-vérifié contre `backend/src/Entity/ScheduleSlotTemplate.php` + `backend/src/Controller/ManualEditController.php` : contrat 2.9 — les 3 champs morts `temporaryLock`/`temporaryLockFor`/`temporaryMinSessionsOverride` disparaissent de l'entité (plus aucun writer depuis le passage CRUD read-only, jamais lus par le solveur), et la route placebo `POST /api/schedule-slots/{id}/manual-edit/constraint` (`ManualEditService::applyPermanentConstraint`) est **supprimée** — la contrainte qu'elle créait portait des clés `config` que le solveur ne lit jamais, contournant `ConstraintConfigValidator`. Zéro ligne `source='manual_edit'` en base. `applyLock`/`move` intacts — §route `manual-edit/lock`) ; précédemment : 2026-08-16 (re-vérifié contre `backend/src/ApiResource/ScheduleSlotTemplateResource.php` + `backend/src/Service/MoveSlotService.php` : le CRUD `schedule_slot_templates` devient read-only (POST/PUT/DELETE + processor/DTO d'entrée supprimés, `/move` est le seul rail d'écriture — via `ManualEditService`/`ScheduleResultImporter` pour `lockOrigin`) et le 422 de `/move` porte désormais les ids du verdict moteur — §ligne 13 et §route `/move`) ; précédemment : 2026-08-16 (re-vérifié contre `backend/src/Service/PdfGenerator.php` : passe esthétique de l'export PDF — pause méridienne teintée, bande de jour, bordure des cellules occupées, heure épinglée en haut + équipes centrées, coach retiré de la grille, matrice équipe×jour en case pleine — §route `export-pdf`) ; précédemment : 2026-08-13 (recalé ce jour par P4-87 : le 3e marqueur de péremption et son listener ; précédemment : 2026-08-12 (recalé ce jour : marqueur `constraintsChangedSinceGeneration` et son listener ; précédemment : 2026-08-12 (recalé ce jour par P4-86 : la route `manual-edit/one-time` est SUPPRIMÉE — `/move` est le seul rail de déplacement ; précédemment : 2026-08-12 (recalé ce jour par P4-84 : la matrice XLSX porte une colonne Rang triable ; précédemment : 2026-08-12 (recalé ce jour par P2-2/F2b : la route `/move` et le marqueur de score périmé ; précédemment : 2026-08-12 (recalé ce jour par P2-2/F1 : `ScheduleSlotTemplate` porte l'ORIGINE de son verrou ; précédemment : 2026-08-11 (recalé ce jour par P2-23 : les routes export-xlsx et export-pdf portent chacune leur 2ᵉ vue équipe × jour ; précédemment : 2026-08-11 (recalé ce jour par P2-23 : l'export XLSX gagne sa 2ᵉ feuille « Équipes × jours » — §route export-xlsx ; précédemment : 2026-08-08 (audit DOC-04, 5e réouverture — re-vérifié au code sur `backend/src/{Entity,ApiResource,Controller,Service,EventListener,Enum}` et `backend/config/`: JWT en cookie httpOnly SEC-16, module démo, approbation de club par token public P3-4, liste blanche `config` des contraintes SEC-13, console superadmin/SEC-17/SEC-18, RLS de `team_tag_assignment` BCK-11, `regenerate`/`regenerate-from`/`export-xlsx`, JWT de souscription Mercure, `TeamLink`/`TeamMatchHabit`))))))))) Plus tôt le même jour : recalé ce jour : **P2-37 PR2 front, LOT SOLDÉ** — les deux lignes ci-dessous (`Reservation`, route `generate`) sont désormais consommées côté écran (`frontend-wizard.md` §4/§5), l'item quitte la roadmap ; précédemment ce même jour, re-vérifié contre `backend/src/Service/PlanVenueClosures.php` + `backend/src/Service/VenueClosureDays.php` + `backend/src/Service/OrphanPinGuard.php` + `backend/src/State/Processor/ReservationStateProcessor.php` + `backend/src/State/Processor/VenuePeriodOverrideStateProcessor.php` + `backend/src/Controller/CalendarEntryConflictsController.php` — **P2-37 PR1 backend + miroir** : gymnase entièrement fermé (indisponibilité DÉRIVÉE des fermetures datées) exclu de l'orphelinage comme un désactivé, réactivation refusée en 422 (POST/PUT/DELETE `VenuePeriodOverride`), réservation refusée en 422 sur gymnase/jour fermé, message `OrphanPinGuard` nommant l'équipe, `fullyClosedVenueIds` sur la route `conflicts` — §lignes `Reservation`/`VenuePeriodOverride`/route `generate`/route `conflicts` ; précédemment : 2026-08-18 (re-vérifié contre `backend/src/Entity/ImplicitRuleSetting.php` + `backend/src/Repository/ImplicitRuleSettingRepository.php` + `backend/src/State/{Provider,Processor}/ImplicitRuleSettingState{Provider,Processor}.php` + `backend/src/Service/SchedulePlanProvisioner.php` — **bien-être PAR PÉRIODE, PR1 backend** : `ImplicitRuleSetting` gagne `schedulePlanId` (ADR-0002 inv. 5), copie matérialisée à la naissance du plan, DELETE en portée plan re-copie la saison — la ressource, absente de ce tableau jusqu'ici, y entre § tableau des ressources ; **PR2 front livrée le même jour** (l'onglet Bien-être exerce désormais la portée par plan — comportement décrit dans `frontend-wizard.md`, la note frontale retirée d'ici) ; précédemment : 2026-08-17 (re-vérifié contre `backend/config/packages/api_platform.yaml` + `App\Service\ProductIdentity` — **P5-15** : le titre OpenAPI reste littéralement `ClubScheduler API`, un résidu hors du périmètre de `ProductIdentityTest` (config/, pas src/) — noté ci-dessous, reste ouvert en roadmap) ; précédemment : 2026-08-17 (re-vérifié contre `backend/src/Service/MoveSlotService.php` + `backend/src/Exception/EngineTimeoutException.php` + `backend/src/Controller/ManualEditController.php` — **correctif d'incident terrain, pas un item de roadmap** : `VALIDATE_HTTP_TIMEOUT_SECONDS` 8 s → **20 s** (maison unique `move()`/`place()`, calé sur mesure) et le service distingue désormais le moteur TROP LENT (`EngineTimeoutException`, timeout traduit depuis `TimeoutExceptionInterface` → **504** `code=engine_timeout`) du moteur injoignable/cassé (502 inchangé) — §route `move`/`place-slot` ; l'incident : un déplacement LÉGAL rendait 502 car le backend abandonnait ~0,7 s avant la réponse du moteur ; trace datée : `etat-des-lieux.md`) ; précédemment : 2026-08-17 (re-vérifié contre `backend/src/ApiResource/SharedTrainingGroupResource.php` + `backend/src/State/{Provider,Processor}/SharedTrainingGroupState{Provider,Processor}.php` + `backend/src/Dto/SharedTrainingGroupInput.php` : **P2-27 PR A, mutualisation** — nouvelle ressource `SharedTrainingGroup` (`/api/shared_training_groups`) — §ligne ci-dessous) ; précédemment : 2026-08-17 (re-vérifié contre `backend/src/Controller/VenueUsageStatsController.php` + `backend/src/Service/VenueUsage/VenueUsageCalculator.php` + `backend/src/Enum/TeamLevel.php` : **P3-22, lot SOLDÉ** — nouvelle route `GET /api/venue-usage-stats` (stats d'utilisation des gymnases par jour, réelles vs à venir, ventilées par gymnase ET par niveau, calcul jour par jour sur `[from,to]`, résolution période validée → vacances (0 h des deux côtés, y compris Réalisé) → grille de saison pointée — §nouvelle section « Statistiques d'utilisation des gymnases ») ; précédemment : 2026-08-17 (re-vérifié contre `backend/src/Message/GenerateScheduleMessage.php` + `backend/src/Controller/RegenerateController.php` + `backend/src/MessageHandler/GenerateScheduleHandler.php` + `backend/src/Service/ScheduleConstraintBuilder.php` : P3-21 PR B, LOT SOLDÉ — le message de génération porte `sourceScheduleId` (version REGARDÉE par `RegenerateController`, pas la dernière du plan), le handler résout le précédent (source explicite de même lignée, sinon repli dernière `COMPLETED` du plan, sinon rien) et l'injecte via `withPreviousAssignments` **après** le hash de snapshot — le précédent est une préférence de convergence, jamais une donnée de structure, sous peine de dé-griser en permanence « Régénérer » — §route `regenerate` et sa note) ; précédemment : 2026-08-16 (re-vérifié contre `backend/src/Controller/ManualEditController.php` + `backend/src/Service/MoveSlotService.php` : P2-32 PR A — contrat 2.10, `MoveSlotService::move()`/`place()` gagnent un paramètre `dryRun` (essai : même chemin jusqu'au verdict INCLUS, gardes pré-moteur et verrou souverain compris, retour AVANT toute écriture — `ManualEditController` répond alors **200** `{valid, dryRun:true, violations, compromises, evicted?}` **même quand `valid=false`**, jamais 422 : un essai ne peut pas échouer au sens HTTP, il RAPPORTE) et les réponses ÉCRITES d'un candidat accepté portent désormais `compromises` (delta de confort nommé du moteur, P2-32 — voir `engine-inventory.md` §POST /validate-assignments) ; timeout **HTTP** de l'appel engine 2 s → **8 s** (`VALIDATE_HTTP_TIMEOUT_SECONDS`, un candidat accepté déclenchant jusqu'à 3 solves moteur — le budget **solveur** par solve reste 2 s) — §route `move`/`place-slot`) ; précédemment : 2026-08-16 (re-vérifié contre `backend/src/Controller/ManualEditController.php` + `backend/src/Service/MoveSlotService.php` : P2-30 PR A — `/move` gagne l'éviction optionnelle (`evictSlotId`, D3 verrou souverain, bloc `evicted`) et le nouveau path `POST /api/schedules/{id}/place-slot` crée une séance à la dérive sous verdict moteur, durée résolue **côté serveur** depuis la fenêtre de gymnase (`durationMinutes` du client = simple assertion, 422 `duration_mismatch`/`slot_unavailable` sinon) — §route `move`/`place-slot`) ; précédemment : 2026-08-16 (re-vérifié contre `backend/src/Entity/ScheduleSlotTemplate.php` + `backend/src/Controller/ManualEditController.php` : contrat 2.9 — les 3 champs morts `temporaryLock`/`temporaryLockFor`/`temporaryMinSessionsOverride` disparaissent de l'entité (plus aucun writer depuis le passage CRUD read-only, jamais lus par le solveur), et la route placebo `POST /api/schedule-slots/{id}/manual-edit/constraint` (`ManualEditService::applyPermanentConstraint`) est **supprimée** — la contrainte qu'elle créait portait des clés `config` que le solveur ne lit jamais, contournant `ConstraintConfigValidator`. Zéro ligne `source='manual_edit'` en base. `applyLock`/`move` intacts — §route `manual-edit/lock`) ; précédemment : 2026-08-16 (re-vérifié contre `backend/src/ApiResource/ScheduleSlotTemplateResource.php` + `backend/src/Service/MoveSlotService.php` : le CRUD `schedule_slot_templates` devient read-only (POST/PUT/DELETE + processor/DTO d'entrée supprimés, `/move` est le seul rail d'écriture — via `ManualEditService`/`ScheduleResultImporter` pour `lockOrigin`) et le 422 de `/move` porte désormais les ids du verdict moteur — §ligne 13 et §route `/move`) ; précédemment : 2026-08-16 (re-vérifié contre `backend/src/Service/PdfGenerator.php` : passe esthétique de l'export PDF — pause méridienne teintée, bande de jour, bordure des cellules occupées, heure épinglée en haut + équipes centrées, coach retiré de la grille, matrice équipe×jour en case pleine — §route `export-pdf`) ; précédemment : 2026-08-13 (recalé ce jour par P4-87 : le 3e marqueur de péremption et son listener ; précédemment : 2026-08-12 (recalé ce jour : marqueur `constraintsChangedSinceGeneration` et son listener ; précédemment : 2026-08-12 (recalé ce jour par P4-86 : la route `manual-edit/one-time` est SUPPRIMÉE — `/move` est le seul rail de déplacement ; précédemment : 2026-08-12 (recalé ce jour par P4-84 : la matrice XLSX porte une colonne Rang triable ; précédemment : 2026-08-12 (recalé ce jour par P2-2/F2b : la route `/move` et le marqueur de score périmé ; précédemment : 2026-08-12 (recalé ce jour par P2-2/F1 : `ScheduleSlotTemplate` porte l'ORIGINE de son verrou ; précédemment : 2026-08-11 (recalé ce jour par P2-23 : les routes export-xlsx et export-pdf portent chacune leur 2ᵉ vue équipe × jour ; précédemment : 2026-08-11 (recalé ce jour par P2-23 : l'export XLSX gagne sa 2ᵉ feuille « Équipes × jours » — §route export-xlsx ; précédemment : 2026-08-08 (audit DOC-04, 5e réouverture — re-vérifié au code sur `backend/src/{Entity,ApiResource,Controller,Service,EventListener,Enum}` et `backend/config/`: JWT en cookie httpOnly SEC-16, module démo, approbation de club par token public P3-4, liste blanche `config` des contraintes SEC-13, console superadmin/SEC-17/SEC-18, RLS de `team_tag_assignment` BCK-11, `regenerate`/`regenerate-from`/`export-xlsx`, JWT de souscription Mercure, `TeamLink`/`TeamMatchHabit`)))))))))
...
| — | Reservation | `/api/reservations` | Réservation d'un créneau de salle pour une équipe (mutualisation : 2 équipes sur un créneau à capacité 2 ; matérialise le verrou pour l'overlay). GetCollection/Get/Post/Delete (pas de PUT — on supprime/recrée) ; ancrable à un plan de période (`schedulePlanId`). **P2-37 (2026-08-18)** : POST refusé en 422 (`ReservationStateProcessor::assertVenueOpen`) si le gymnase est ENTIÈREMENT fermé sur la fenêtre (fermeture datée dérivée, `PlanVenueClosures`) ou si le jour visé est un jour fermé — cause nommée. Une réservation DÉJÀ posée n'est jamais supprimée ni déplacée par une fermeture ultérieure (décision fondateur : alerter, pas modifier passivement) ; l'alerte vit dans le prédicat partagé `OrphanPinGuard::unservedReservationIds` (miroir front `unservedReservationIds`, câblé au récapitulatif du wizard depuis P2-37 PR2, 2026-08-18 — détail `frontend-wizard.md` §5). | |
| — | SharedTrainingGroup | `/api/shared_training_groups` (filtre `schedulePlanId` exact) | P2-27 — déclarer que N équipes (2..10) s'entraînent ENSEMBLE, avec EXACTEMENT `commonSessions` séances communes (consommé par le solveur — `sharedTrainings` du payload, contrat 2.12, ≠ `Reservation` qui matérialise un verrou statique). GetCollection/Get/Post/Put/Delete. `schedulePlanId` nullable (NULL = socle saison, non-null = plan de période — ADR-0002 inv. 5, un plan de période **naît sans déclaration**, patron `Reservation`). 422 métier (processor) : équipe déjà dans un AUTRE groupe de la MÊME portée, `commonSessions` > le plus petit `sessionsPerWeek` effectif des membres (override de période inclus). Supprimer une équipe la retire de ses groupes et élague tout groupe tombé sous 2 membres (`EntityCascadeDeleter`) ; purgé avec le plan (`OverlayManager::purgePlanAnchoredSettings`) et avec la saison (`SeasonDataPurger`) ; toute écriture marque les plannings `COMPLETED` du bon périmètre périmés (`ResourceChangeStaleScheduleListener`). | |
| — | VenuePeriodOverride | `/api/venue_period_overrides` (+ actions atomiques `POST /reset-grid` « reprendre la grille du planning principal » et `POST /clear-grid` « vider la grille » pour un gymnase — SEC-07, 422 si visées sur le plan de saison, 404 hors club) | Mode d'un gymnase pour une période (#8) : sparse par (`schedulePlanId`, `venueId`), `DISABLED`\|`BLANK` — pas de ligne = hériter la grille de saison. DÉSACTIVÉ conserve la grille mais sort le gymnase du payload engine ; VIERGE la vide ; DELETE (retour à hériter) la revide puis la recopie. **P2-37 (2026-08-18)** : si le gymnase est ENTIÈREMENT fermé sur la fenêtre par une fermeture datée (indisponibilité DÉRIVÉE, `PlanVenueClosures`), POST/PUT **et DELETE** sont refusés en 422 nommant la fermeture — le DELETE aussi, sinon « revenir à hériter » réactiverait le gymnase par la petite porte. | |
...
| — | CalendarEntry | `/api/calendar-entries` | Cockpit temporel : périodes/événements (kind PERIOD/EVENT ; `parentEntryId` = semaine ENFANT d'une mère découpée). L'entrée porte le **FAIT** ; la RÉPONSE vit sur le plan — `overlayScheduleId` a été **supprimé** (ADR-0002 lot D-b), « période → version active » se dérive de `SchedulePlanProvisioner::chosenOfPeriodPlan`. **Une semaine-enfant naît AVEC son plan** — soumise à la même garde `PeriodWindowUniquenessGuard` que le POST `schedule_plans` (P2-38 PR2, 2026-08-18) : 409 `window_already_planned` si sa fenêtre recoupe déjà un AUTRE plan de période. Déclarer une entrée RACINE (le FAIT — même une fermeture par-dessus une période déjà planifiée) reste, elle, toujours libre : la garde ne s'applique jamais à ce site. | Opération custom conflits (§3) |
...
|-------|---------|------------|-------------|
| `/api/schedules/{id}/generate` | POST | `GenerateScheduleController` | Lance la génération asynchrone. Gate management (`assertManager`, SEC-07). Vérifie l'appartenance du schedule au club courant, **borne de complexité A10 pré-dispatch** (`GenerationComplexityGuard` : teams ≤200 · venues ≤50 · slots ≤3000 · contraintes permanentes ≤500 · teams×venues ≤2000 → **422** avant toute mise en queue, statut inchangé, #156), **épinglage orphelin sur un planning de période** (`OrphanPinGuard`, #8) : un verrou HARD ou une réservation qui ne retombe plus sur aucun créneau de la grille de la période (grille refaite : page blanche, recopie) → **422** nommant le gymnase, le jour et l'équipe (nom d'équipe ajouté par P2-37, 2026-08-18). ⚠ **Un gymnase DÉSACTIVÉ (P3-20, 2026-08-06) ou ENTIÈREMENT FERMÉ sur la fenêtre par une fermeture datée (P2-37, 2026-08-18 — indisponibilité DÉRIVÉE des dates fermées, `PlanVenueClosures`/`VenueClosureDays::fullyClosedVenueIds` ; deux fermetures qui se RELAIENT sans qu'aucune ne couvre seule la fenêtre comptent quand même) en est exclu** : son épinglage est inerte (le solveur ne le verra jamais) et revient intact à la réactivation — refuser enfermait le gestionnaire sur un épinglage devenu invisible. **Une fermeture PARTIELLE (un seul jour fermé d'un gymnase par ailleurs ouvert) reste bloquante** : là, la séance serait replacée ailleurs en silence, ce que ce garde existe pour empêcher. Passe le statut à `PENDING`, marque `onboardingCompleted=true` à la première génération, dispatche `GenerateScheduleMessage`. Retourne 202. |
