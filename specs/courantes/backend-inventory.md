...
| 13 | ScheduleSlotTemplate | `/api/schedule-slot-templates` | Créneaux générés | **`lockOrigin` depuis P2-2/F1 (2026-08-12)** — `RESERVATION` \| `MANUAL` \| `UNKNOWN`, **nullable** (`NULL` = pas de verrou : les 3 valeurs ne portent que sur un verrou RÉEL). **Server-authoritative** : absent de `ScheduleSlotTemplateInput`, aucune route ne le pose. Écrit aux 4 points d'origine — import du résultat solveur (`ScheduleResultImporter`), épinglage work-loop (`ManualEditService::applyLock`), CRUD management (`ScheduleSlotTemplateStateProcessor`), pseudo-créneaux de réservation côté front. ⚠ **`UNKNOWN` dit une IGNORANCE, pas une absence de verrou** — un verrou HARD sans réservation appariée reste indécidable et n'est **jamais deviné** (gardé par `LockOriginProvenanceTest`, **step de `blocking-tests`**). Le backfill de migration respecte **quel plan chaque réservation alimente** (base `NULL` → SEASON, overlay → son plan) pour ne pas fabriquer de faux `RESERVATION`. |
...
| `/api/schedule-slots/{id}/move` | POST | **Déplace un créneau SOUS LE VERDICT DU MOTEUR** (P2-2 F2b, 2026-08-12) — `MoveSlotService` : (1) **409** `generation_in_progress` si `ClubGenerationLock::isGenerating()`, (2) baseline construite **sans la source ET sans les créneaux des versions sœurs**, (3) `POST /validate-assignments` sur l'engine, (4) refus → **422** `{valid:false, violations:[{rule, message}]}` (messages déjà humains) et **rien n'est écrit**, (5) accord → écriture + marqueur `manuallyEditedSinceGeneration` + publication Mercure. Moteur injoignable → **502**, rien écrit. Planning validé → 409 (lecture seule). ⚠ **La re-validation a lieu AU MOMENT D'ÉCRIRE** — le verdict n'est jamais un cache. NR `SlotMoveVerdictTest`, **step de `blocking-tests`**. |
| `/api/schedule-slots/{id}/manual-edit/lock` | POST | Applique un verrou sur un créneau. Body : `lockLevel` (enum `LockLevel`). Retourne 200. |
| `/api/schedule-slots/{id}/manual-edit/one-time` | POST | Applique une modification ponctuelle sur un créneau (ex. `startTime`). Retourne 200. Conflit → 409.  ⚠ **ORPHELIN de l'application depuis F2b (2026-08-12)** : le déplacement passait par ici et passe désormais par `/move` ; plus aucun appelant frontend, seuls des tests l'exercent. Non supprimé (décision d'API) → roadmap. Sa dette subsiste : `applyOneTimeUpdate` **mute le template en permanence** malgré son nom, et son contrôle ignore capacité/fenêtres/repos.|
