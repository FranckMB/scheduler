...
1. **Équipes** — CRUD + classement : groupées par tier (`PriorityTier` S/A/B/C/D), ordre **intra-tier** via `Team.tierOrder` (monter/descendre) ; le numéro global dérive de (tier, tierOrder). Le **niveau** s'affiche en toutes lettres (« S · Fanion » … « D · Bonus », mapping front `TIER_MEANING`) ; genre M/F/MIXTE affiché Homme/Femme/Mixte (libellés front, valeurs backend inchangées). Validation : ≥1 équipe.
