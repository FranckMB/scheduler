...
Last verified @ 2026-08-18 (recalé ce jour par **P2-37 PR2 front, LOT SOLDÉ** — §4 (« Réserver ») et §5 (Récapitulatif) : un gymnase ENTIÈREMENT fermé sur la fenêtre (`fullyClosedVenueIds`, donnée serveur) se distingue désormais d'un gymnase simplement désactivé — libellé « fermé cette période » dans le sélecteur, bandeau et messages de modale alignés sur le refus serveur (D3) ; le récapitulatif liste les réservations NON SERVIES (prédicat large `unservedReservationIds`, miroir déclaré) en nommant l'équipe ET le motif (« gymnase fermé — titre » / « gymnase désactivé pour cette période » / « créneau supprimé ou déplacé »), sans en supprimer aucune d'office (décision fondateur : on alerte, on ne modifie pas passivement) — re-vérifié contre `ReservationPanel.tsx`, `SlotReservationModal.tsx`, `RecapStep.tsx`, `queries.ts::useActiveVenues`. Précédemment recalé le même jour par **P2-35 PR2 front, LOT SOLDÉ** : l'onglet « Bien-être » devient conscient de la période — en mode période, le panneau règle et affiche la COPIE du plan (`schedulePlanId`) derrière `PeriodAnchorGate`, même ancrage que « Réserver »/« Mutualisation » ; la portée voyage dans le GET/PUT/DELETE et dans la clé de cache react-query ; phrase de portée + repère « Saison : … » sous chaque règle ; « Réinitialiser » re-copie la valeur de saison en portée période (jamais la suppression) — re-vérifié contre `ConstraintsStep.tsx`, `ImplicitRulesPanel.tsx`, `wizard/api.ts`, `wizard/queries.ts` ; précédemment : 2026-08-17, **P2-27 PR B, lot SOLDÉ** : l'étape Contraintes gagne l'onglet « Mutualisation » (`MutualisationPanel.tsx`) — déclarer que N équipes s'entraînent ensemble avec K séances communes, classement d'affichage `rankCandidates`/`lib/sharedTraining.ts` (même catégorie + genre compatible, jamais un ordre d'âge), avertissement non bloquant au-delà de 3 équipes, pré-validation FAIL-SAFE devant le verdict serveur ; le repère « Mutualisée avec … » gagne les étapes Équipes (socle) et Équipes de période ; l'accordéon « Mutualisation » gagne le Récapitulatif ; précédemment : 2026-08-15, P2-28 PR 3 (onglets « Base »/« Bien-être ») ; 2026-08-14, cohérence canSplit (modale d'impact + cascade), P2-22 PR 2 (fermetures au grain JOUR), P2-17 PR 2 (`GroupLabelField`) ; 2026-08-11 : P4-55 ; 2026-08-08 : retrait `FACILITY_CAPACITY`)
...
- **3 modes, mêmes écrans** : *libre* (club ayant déjà généré) · *onboarding guidé* (nav verrouillée vers l'avant) · **mode période (palier B)** = adaptation d'un `CalendarEntry`. Le mode vit dans le store wizard (`mode`, `calendarEntryId`, `startPeriodMode`/`exitPeriodMode`, **persist v4**), déclenché par le cockpit (radar « Adapter » → `startPeriodMode(entryId)` + navigate `/wizard`, atterrissage sur l'étape **Contraintes**).
  ...
  - **Mode période — la période POSSÈDE sa grille (#8, 2026-07-24)** : les `VenueTrainingSlot` de la période sont une **copie** du modèle de saison prise à la naissance du plan ; la construction de l'overlay ne s'unit **jamais** aux créneaux de la saison. Conséquence directe sur les écrans :
    ...
    - **Équipes** : activables / désactivables pour la fenêtre (`PeriodTeams`). **Coachs** : hérités en lecture seule (`ReadonlyCoaches`). **Repère « Mutualisée avec … »** (P2-27 PR B) posé aussi ici, sur les groupes de **LA PÉRIODE** (`SharedTrainingGroup` du `schedulePlanId` de l'entrée) — le poser seulement au socle mentirait par omission en reprise, précisément le terrain que P2-27 sert.
