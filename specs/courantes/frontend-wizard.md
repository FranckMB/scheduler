...
Last verified @ 2026-07-29 (mode période re-vérifié contre `frontend/src/features/wizard/`)
...
- **3 modes, mêmes écrans** : *libre* (club ayant déjà généré) · *onboarding guidé* (nav verrouillée vers l'avant) · **mode période (palier B)** = adaptation d'un `CalendarEntry`. Le mode vit dans le store wizard (`mode`, `calendarEntryId`, `startPeriodMode`/`exitPeriodMode`, **persist v4**), déclenché par le cockpit (radar « Adapter » → `startPeriodMode(entryId)` + navigate `/wizard`, atterrissage sur l'étape **Contraintes**).
  - **Mode période — chrome** : bandeau « Mode période — {titre} · fenêtre » + « Quitter ». Le bandeau porte aussi un bouton **« Doléances »** (#10 — ouvre la todo-list des doléances coachs, filtrée sur la semaine du plan courant ; affiché seulement si la période s'y prête et qu'une entrée mère existe) et un bouton de **suppression du planning de période** (hors étape Génération).
  - **Mode période — la période POSSÈDE sa grille (#8, 2026-07-24)** : les `VenueTrainingSlot` de la période sont une **copie** du modèle de saison prise à la naissance du plan ; la construction de l'overlay ne s'unit **jamais** aux créneaux de la saison. Conséquence directe sur les écrans :
    - **Gymnases : ÉDITABLE**, ce n'est plus un résumé en lecture seule (`PeriodVenues`). Un **sélecteur de gymnase** (un panneau à la fois, exactement comme en saison — pas « une carte par gymnase, toutes montées », qui avait fait ressurgir durée figée, absence d'éditeur, mur de boutons et clavier cassé), pose de créneau au clic (90 min par défaut, ajustables dans la modale d'éditeur partagée), anti-chevauchement, suppression. Un gymnase **fermé par contrainte** est badgé **INTERDIT cette période** (via l'endpoint conflits). Un créneau posé sur un jour que la grille n'affiche pas (dimanche, hérité de l'ancien formulaire) est rendu **visible et supprimable** plutôt que servi en silence au solveur.
