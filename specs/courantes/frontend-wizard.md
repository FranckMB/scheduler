...
Last verified @ 2026-08-14 (recalé une 3ᵉ fois ce jour par la cohérence canSplit : modale d'impact + cascade au décochage de « Terrain divisible » sur un gymnase à créneaux ≥ 2, 422 filet sans flag, warning Réserver qui nomme la divisibilité — re-vérifié contre `VenuesStep.tsx`, `reservationSlots.ts` (`splitCascadePreview`), `useStepValidation.ts`, `VenueStateProcessor.php` ; précédemment recalé ce jour par P2-22 PR 2 : les fermetures de gymnase se DISENT au grain JOUR sur les grilles (Gymnases/Réserver), `PeriodStructure` et le récap — remplace l'ancien badge tout-ou-rien « INTERDIT cette période » — re-vérifié contre `frontend/src/features/wizard/lib/venueClosures.ts`, `VenueAvailabilityGrid.tsx`, `ReservationGrid.tsx`, `ReservationPanel.tsx`, `SlotReservationModal.tsx`, `PeriodStructure.tsx`, `RecapStep.tsx`, `ConstraintsStep.tsx`, `constraintOrder.ts` ; précédemment recalé ce même jour par P2-17 PR 2 : `GroupLabelField` sous le hint de créneau partagé (étape Gymnases) + badge de groupe sur `ReservationGrid` (étape Contraintes → Réserver) ; précédemment : 2026-08-11 (recalé ce jour par P4-55 : **encart lecture seule des six règles structurelles** en tête de l'étape Contraintes, et retrait du raccourci « coach jamais en double » devenu faux depuis D-14 ; précédemment : retrait de `FACILITY_CAPACITY` 2026-08-08 · un solve FAILED qui s'explique à l'écran · terrain divisible jusqu'à 3 équipes · « Associer à… » les salles FFBB · contraintes de base à la création P2-16 · fenêtres d'accès match sur la grille · durée à la pose en période P4-43 ; vérification directe du 2026-08-01 : étape Gymnases re-vérifiée contre `frontend/src/features/wizard/` — grille 8h→23h sur sept jours, durées 45 min→2h30, P4-37 ; bandeau de mode période et diagnostics de l'étape Génération re-vérifiés contre `WizardLayout.tsx` et `PlanningPage.tsx`, P4-38/P4-40))
