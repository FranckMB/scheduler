...
- **2 modes, mêmes écrans** : *libre* (club onboardé) vs *onboarding guidé* (nav verrouillée vers l'avant) selon `club.onboardingCompleted`, exposé dans `/api/me`. Bascule à `true` **au lancement** de la 1ère génération (`GenerateScheduleController`).
