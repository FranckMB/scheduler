...
Last verified @ 2026-08-19 (**rotation de fraîcheur** — re-vérifié contre le code, tout juste : `Club.logoUrl`/`accentColorDark`/`accentPalette` existent (`backend/src/Entity/Club.php`), `PATCH /api/club/appearance` existe (`ClubAppearanceController.php:33`), `useApplyClubTheme.ts:16-18` lit bien `accentColor`/`accentColorDark`/`accentPalette` sur `me.club`, et `GenerationWaiting.tsx` ne prend plus de prop `initial`/`logoUrl` — aucune occurrence trouvée) ; précédemment : 2026-08-18 (aucun changement de FOND : seuls les chemins vers `frontend-spec.md` sont recalés, ce fichier ayant rejoint `frontend/docs/` — les affirmations sur la marque et la scène d'attente sont inchangées et restent vraies) ; précédemment : 2026-08-17 (re-vérifié contre `GenerationWaiting.tsx`/`GenerateStep.tsx`/`PlanningPage.tsx` : l'écran d'attente de génération ne montre plus le logo/l'initiale du club — voir décision ci-dessous)
