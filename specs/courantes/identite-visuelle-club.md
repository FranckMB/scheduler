...
Last verified @ 2026-08-22 (**rotation de fraîcheur** — re-vérifié contre le code, tout juste : `Club.logoUrl`/`accentColorDark`/`accentPalette` dans l'entité (`backend/src/Entity/Club.php`) ✓, `PATCH /api/club/appearance` (`ClubAppearanceController.php:33`) ✓, `useApplyClubTheme.ts:16-18` lit `accentColor`/`accentColorDark`/`accentPalette` sur `me.club` ✓, et `GenerationWaiting.tsx` n'affiche toujours aucun logo/initiale de club — sa seule occurrence de « logo » est le commentaire qui acte la décision fondateur du 2026-08-17 ✓. **Passe aussi corrective** : ce stamp s'était RE-empilé à 4 entrées « précédemment » depuis l'aplatissement DOC-33 du 2026-08-19 — aplati de nouveau ; l'historique vit dans git : `git log -p --follow specs/courantes/identite-visuelle-club.md`)
