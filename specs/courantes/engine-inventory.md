...
**Bornes A10** (#156, anti-bombe de génération) : chaque liste porte un `max_length` (rejet **422** avant CP-SAT) — `teams` ≤200 · `venues` ≤50 · `coaches` ≤200 · `constraints` ≤500 · `slot_templates` ≤2000 · `priority_tiers` ≤20 · `trainingSlots` ≤1000/gymnase ; plus un `model_validator` bornant le **total** des créneaux à ≤3000 (empêche 50×1000). Le backend (`GenerationComplexityGuard`) pré-vérifie **un sous-ensemble** de ces caps (teams, venues, coaches, contraintes permanentes, total créneaux `MAX_AVAILABILITY_SLOTS`=3000) **plus** `teams×venues` ≤2000, **avant dispatch** ; `slot_templates`/`priority_tiers` ne sont bornés que côté engine (max_length). ⚠ résidu **ENG-23** : le backend compte les contraintes **brutes** (permanentes) tandis que l'engine plafonne le tableau **étendu** (CLUB→N TEAM) → un club à ~500 contraintes permanentes peut passer le guard puis prendre un 422 engine. ⚠ Ce durcissement de validation n'a **pas** bumpé `CONTRACT_VERSION` (2.0) : politique — un `max_length` resserre l'enveloppe acceptée sans changer forme/type ni MAJOR ; un bump n'est requis que pour un changement de forme/sémantique (champ/type/alias).
