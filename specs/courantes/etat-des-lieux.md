...
|------|----|-------|----------------|
| 2026-08-01 | P3-15 (a)(b) — **P3-15 SOLDÉ** | **La modale de sollicitation redevient lisible** — « BEAUCOUP TROP longue ». ⚑ **Recadrage fondateur** : le problème n'était pas la modale mais qu'elle affichait TOUTES les équipes, une par ligne (49 sur un club réel) ; les onglets étaient une piste, pas le remède. D'où trois leviers, par ordre d'effet : le choix des équipes se **replie derrière une ligne** et se déplie en **puces enroulées groupées par rang**, avec des gestes de masse ; une nouvelle collecte démarre avec **toutes** les équipes ayant un coach (le cas courant passe de 49 clics à zéro) ; la liste des coachs va dans un **second onglet**. ⚑ Le composant d'onglets a été **déplacé** de `features/admin/` vers `shared/components/ui/tabs.tsx` avec deux peaux plutôt que dupliqué — la console ne bouge pas d'un pixel (`variant="console"` reprend ses classes verbatim). ⚠ Piège évité en chemin : le défaut « toutes les équipes » est **dérivé**, pas figé dans un `useState` initial — `teams` arrive après le premier rendu, et un état initial l'aurait laissé vide pour toujours | [`types-de-planning.md`](types-de-planning.md) E5 |
