...
- **État** : 🟡 partiel — le moteur reprise existe (holiday : héritage contraintes +
  ...
  (`isAdaptableHoliday`). Le défaut équipes est **Fanion + importantes** (E3 ✅). Voir « Écarts » ci-dessous.
...
| E6 | **Nommage des plannings** — souci de **conception**, pas de nommage : le nom « du plan » n'a pas de conteneur (il vivait à tort sur la `Season` pour le socle, nulle part pour l'overlay). Une 1re tentative (nom sur `Schedule.name`) a échoué : `Schedule` = une **version**, pas le plan → divergences baseline/dernière-version. **Repris comme pattern `Plan` premier-ordre → roadmap P2-6.** | 1 + 2 + 3 | Entité `Plan` (nom public + `chosenVersionId`) ; `Schedule` = version auto « Vn - date » ; en-tête/liste/export = nom du Plan. Défauts : socle `Planning de la saison 20XX-20XX`, indispo `Ajustement {gym} du…au…`, reprise `Planning de vacances de {nom} du…au…` |
